...
a = a[:start] + '''        public bool TryAcquire(int vertexCount, int indexCount, out SurfaceGeometryLease lease)
        {
            lease = default;
...
s = s[:start] + '''            private bool EnsureUploadStaging(int vertexCount, int indexCount)
            {
...
pressure = '''            // Arena exhaustion is backpressure, never a reason to allocate another GPU
...
'''
anchor = '            _water.InvalidateSurfaceBricks(storage, _discoveredSurfaceBricks);\n'
q = once(q, anchor, pressure + anchor, 'pressure policy anchor')
