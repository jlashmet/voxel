...
- [x] Validate segment/chunk/LOD road geometry/material continuity in the built player: run `33296430341` survey shows continuous outer/town roads across streamed terrain without visible road breaks or repeated shoulder bands.
...
- [x] Obtain a green combined exact-source PlayMode + built-player request through the same `ci-test/fixes/agent-1` transport with visually sufficient road evidence: final run `33296430341` on request `ecdcafbb50810e5944858b2035b9e47d7bfb7c95` (source parent `ca9174082c2f3ecea78dfee0e503ff17fb6711a2`) passed 2/2 PlayMode tests, real-player capture, artifact upload, and final status.
- [x] Run/inspect final repaired built application/player evidence for `Assets/Scenes/KentridgePlayableSlice.unity`; player ran 60.4s and exited with zero harness assertion failures or runtime exceptions.
- [x] Capture/inspect endpoint-to-endpoint road continuity and player traversal with collision/streaming active: t=39.3s and t=49.3s are active autowalk frames; t=59.3s fixed survey shows the connected town/outer road network alongside the source-backed world-layout route overlay.
- [x] Inspect both shoulders on uneven/sloped terrain for natural Grass↔Dirt recovery with no repeated bands, checker staircase, exposed wall, or hard binary shoulder pattern; the t=49.3s walking frame shows the Dirt corridor and both grassy recoveries across uneven terrain.
- [x] Inspect medium/far views for chunk/LOD seams and floating props: the t=59.3s fixed survey retains continuous visible road geometry/material across the streamed view with no road seam or disconnected segment.
- [x] Verify vegetation suppression/recovery and semantic route/influence traceability evidence in the built application: the carriageway is visually bare while grass resumes outside it; runtime reported 11,322 ground-cover instances and 1,694 route-excluded candidates, while the on-screen `SOURCE-BACKED WORLD LAYOUT — KENTRIDGE` overlay retained the authoritative route graph.
- [x] Quantify bounded cost: final PlayMode Unity invocation peak RSS 5591 MB with 0 MB swap growth; real-player FPS settled above ~60 fps and reached 100–160 fps during later traversal/survey samples; survey streaming reached 64 resident regions with 148,210/262,144 mixed residency entries, 9 evictions and 29,659 reclaimed mixed entries. Runtime exposed 20 source-backed macro routes; the green bounded-catalogue regression enforces `<= FeatureBudget.MaxDefinitions`, one `EmitTerrainCorridor` primitive per definition, zero legacy road boxes, bounded footprints, unchanged persisted voxel size/32-byte vertex stride, and no per-frame road generation.
