...
- [ ] Validate the streaming-coverage radius regression on the current exact feature SHA through `ci-test/fixes/agent-6` with the required 180-second SceneIssue replay.
- [ ] Require strict built-player coverage to advance beyond Moordell after the coverage-radius fix; if the same symptom remains, isolate the new failure boundary before another fix.
...
- [x] Latest pre-coverage-fix replay preserves Moordell readiness around 85 s with 29 in-radius baseline residents and zero feature-only vertical extras.
...
- [x] (9) Focused behavioral regressions cover determinism, reachability, roads, settlements, constraints, blocked-route failure, spatial reservations, bounded water cost, evidence sequencing, vertical residency/readiness, settlement framing, hollow-shell/plinth cost, and runtime publication/storage discriminators. The previously outstanding perimeter-foundation assertion is green in exact-SHA run `33800775569`; the new independent coverage-radius regression is an additional correctness guard for the remaining visual blocker, not a reopening of this completed clause.
