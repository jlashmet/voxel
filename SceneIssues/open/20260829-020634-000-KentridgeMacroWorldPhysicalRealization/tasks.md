...
- [ ] Validate `KentridgeMacroWorldSurveyStreamingAlignmentTests.ElevatedSurveyPinsStreamingDemandToRenderedCameraBeforeSliceStreaming` on the current exact feature SHA through `ci-test/fixes/agent-6` with the required 180-second SceneIssue replay.
...
- [x] (9) Focused behavioral regressions cover determinism, reachability, roads, settlements, constraints, blocked-route failure, spatial reservations, bounded water cost, evidence sequencing, vertical residency/readiness, settlement framing, hollow-shell/plinth cost, and runtime publication/storage discriminators. The perimeter-foundation assertion is green in exact-SHA run `33800775569`; the independent coverage-radius regression is green in `33804821454`; the new survey-demand alignment regression is pending exact-SHA validation.
