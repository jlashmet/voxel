...
- [x] Classify CI run `33522587461` as test-harness compile red only: its source lacked `Game.WorldBuilder.Macro.Api` / `.Runtime` imports, so the product path never executed. Imports are corrected on the feature branch.
