...
- [x] Re-fetch renderer correction state: `fixes/agent-1=fc767620a0fe5d0dfee204947d13e7eefaa2a3fa` is a clean continuation whose sole parent is current master `da2617e12c392f86c4bfe1ed3af24c8f8e754056`; it preserves only far-presentation CPU-diagnosis work/module-local regressions and adopts master-owned Showcase Input System/runtime validation.
...
- [x] Pin the active fail-before request without replacement: `ci-test/fixes/agent-1=6ddc72724c6653538be5c5a9818ebee059726264`, parent/source `da3f5be338c57f5fe99ad4324405422e78c3918e`, run `33999899224` is `queued`. Do not replace or interfere while queued/running.
...
- [ ] After Agent 1 completes the fail-before/pass-after discriminator, clears CPU visual acceptance, validates the renderer/GPU correction, and merges it to `master`, merge then-current `origin/master` into `fixes/agent-6` per `master-sync-required.md` and re-evaluate strict opening/publication convergence.
