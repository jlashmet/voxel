...
- [x] Merge the latest repository guidance from master via PR #249; feature merge `d8476f7b3ef1c316e4c30fee8448ec86e8746dc1` includes master `13b3c6a752deb030effba0f6e430863d0c1fd115` and the mandatory module-local validation-scene rules.
...
- [ ] `Assets/Game/WorldBuilder/Generation/Validation/MacroPhysicalWorld/MacroPhysicalWorldValidation.unity` (+ scenario/probe as needed): exercise the production `MountingForce.WorldGen.Voxel` physical planner/catalogues with deterministic generic settlements, hard routes, blocked geography, substantial water, and ridge/pass constraints. It must render production-authored output rather than test primitives and prove the physical-realization path independently of Kentridge composition.
- [ ] `Assets/Game/Composition/Showcase/Validation/FeatureResidency/FeatureResidencyValidation.unity` (+ scenario/probe): exercise production `ShowcaseWorld` feature-aware vertical residency/readiness with an authored multi-region-height feature and verify upper authored layers become resident without widening horizontal radius.
- [ ] `Assets/Game/Composition/Kentridge/Playable/SceneRuntime/Validation/KentridgeMacroWorld/KentridgeMacroWorldValidation.unity` (+ scenario/probe): exercise the production Kentridge SceneRuntime composition and source-backed macro selection in a focused deterministic scene; validate at least one grounded settlement/road/geography relationship and the same streaming/readiness path used by the shipped slice. The top-level `KentridgePlayableSlice` integration scene does not satisfy this checkbox.
- [ ] `Assets/VoxelEngine/Rendering/Validation/GpuSurfaceMirrorRelocation/GpuSurfaceMirrorRelocationValidation.unity` (+ scenario/probe): exercise the real GPU voxel mirror/extraction/publication path, perform a controlled distant relocation, and assert that recovery/admission resumes rather than leaving all worker demands stage-admission pending.
...
- [ ] Run repository-derived module validation after all four owned scenes are present and inspect their built-player evidence/diagnostics directly; scene existence alone is not acceptance.
...
- [ ] (9) Focused behavioral regressions cover determinism, reachability, roads, settlements, constraints, blocked-route failure, spatial reservations, bounded water cost, evidence sequencing, vertical residency/readiness, settlement framing, hollow-shell/plinth cost, runtime publication/storage discriminators, GPU count-batch liveness, distant-relocation mirror liveness, and required module-local player validation. The relocation regression and new owned scenes still require exact validation.
