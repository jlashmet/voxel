...
- [ ] Add executable required macro-specific validation scene/scenario pairs for WorldBuilder macro physical output, Showcase feature residency, Kentridge playable macro world, and current-renderer GPU mirror relocation. The 2026-09-07 reconciliation audit found the four named scene paths have no branch history, so the prior checked claim was stale rather than work lost during the master merge.
