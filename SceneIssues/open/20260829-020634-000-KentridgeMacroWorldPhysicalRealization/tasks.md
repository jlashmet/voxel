...
- [x] Re-fetch renderer correction state: `fixes/agent-1=9a63e86d64e20aaa9cbdd27d8bb0f240f3b011ef` is cleaned/rebased onto current-master Input System authority. Agent 1 moved composition NUnit coverage into module-local editor-only `VoxelEngine.Composition.Tests.EditMode`; its plan identifies far primitive geometry loss/AABB fallback as the next CPU visual discriminator. `ci-test/fixes/agent-1` still points to old failed request `3fc980e...`, so `9a63e86...` is not exact-SHA validated and is not on master.
