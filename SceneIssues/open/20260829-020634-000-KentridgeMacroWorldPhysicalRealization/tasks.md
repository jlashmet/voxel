...
- [x] Re-fetch renderer correction state: `fixes/agent-1=fc767620a0fe5d0dfee204947d13e7eefaa2a3fa` is a clean continuation whose sole parent is current master `da2617e12c392f86c4bfe1ed3af24c8f8e754056`; it preserves only far-presentation CPU-diagnosis work/module-local regressions and adopts master-owned Showcase Input System/runtime validation. Agent 1's plan still identifies far primitive geometry loss/AABB fallback as the next CPU visual discriminator. `ci-test/fixes/agent-1` still points to old failed request `3fc980e...`, so `fc767620...` is not exact-SHA validated, has not cleared CPU visual acceptance, has no open PR, and is not on master.
