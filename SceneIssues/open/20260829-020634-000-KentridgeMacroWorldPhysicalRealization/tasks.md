...
- [x] Add all-building water/separative-region footprint regression and remediate the proven Orc/Southern-Ridge overlap by bounding the modern ridge blockout while preserving authored route solutions.
...
- [ ] In the dormant `kentridge-macro-world` evidence profile only, dismiss pending opening dialogue promptly while retaining normal production opening behavior outside validation.
- [ ] Hold each production-derived settlement survey demand while all four building-centre presentation columns settle; do not serially move demand between building centres.
...
- [x] Current feature branch includes `origin/master` `0901be5a0640e3eec103cdf3c97aa12b8cd42a9e`; prior comparison showed only this Kentridge feature/tests/docs beyond master.
