...
- [ ] Validate the perimeter-foundation throughput fix on the current exact SHA. Runs `33635511188` and `33635715141` were runner-memory infra failures. Retry 5 `33641059051` had healthy admission (38,665 MB free) but repository-derived validation failed first in 15 unrelated renderer/GPU EditMode tests caused by stale branch/master baseline drift, so the requested process-isolated Kentridge PlayMode regression never executed. Do not modify renderer code/tests or claim this checkbox green; see `experiment-030-throughput-retry5-baseline-renderer-gate.md`.
- [ ] Re-run built-player authored shell/roof probes and require Fairy/Orc storage plus readable captures after the throughput fix. Retry 5 independently proves Moordell content-ready by ~85 s and correct 480-definition macro ownership, but published renderer coverage stayed false through 180 s so strict evidence capture never advanced beyond Moordell readiness.
...
- [ ] Measure final feature work/time, CPU/FPS, memory, streaming convergence, render/far-field telemetry against budgets. Retry 5 provides partial evidence: post-30 s median one-second FPS ~103.9 across 143 samples with burst stalls; FAR coverage remained false. This is not final closure validation.
- [ ] Quantify final fixed-replay target timing. Retry 5 improves Moordell content-ready from pre-fix ~175 s to ~85 s (about 90 s faster), but renderer coverage blocked capture/advance and later targets did not complete.
