...
7. **Preserve both the authoritative hard graph and the substantial Logan ridge, and explicitly skirt the ridge shoulder on the Orc route.** Selected. Use the existing deterministic `GoAround` semantic targeted at `southern-ridge`, with a focused regression that the resulting travel corridor remains outside the ridge margin and is walkable.
...
## Current repair gate
Add exactly one Orc-route semantic solution in `KentridgeTopDownWorldPhysicalIntent`: `south-fighting-area-1 -> orc-village` uses `GoAround` targeting `southern-ridge`, so the planner automatically chooses a dry/walkable shoulder detour while retaining the exact verified graph and the Logan pass. Add a focused production regression that proves the solution is authored against the ridge and the planned Orc corridor stays outside the ridge travel margin. Do not change the generic planner, resize/reposition the ridge, add a second pass, or modify catalogue/water behavior.
