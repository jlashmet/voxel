...
Agent 1 rebuilt the continuation directly on current master at `fixes/agent-1=fc767620a0fe5d0dfee204947d13e7eefaa2a3fa`, whose sole parent is `da2617e12c392f86c4bfe1ed3af24c8f8e754056`. It preserves only the renderer SceneIssue's far-presentation CPU-diagnosis work and module-local regressions while adopting master-owned Showcase Input System/runtime validation. Agent 1 has now issued exact transport `8e6aac9fe8845a04a0bdfca2640fc11988e50506`, parented directly to `fc767620...`; run `33996360570` is in progress. Automatic module validation is still running and the SceneIssue standalone replay has not started. Leave this exact request untouched. Agent 1's plan still identifies canonical primitive geometry being reduced to AABB/box/cylinder fallback as the leading cause of the giant far proxy, so even a green build/test prerequisite is not enough until CPU built-player visual acceptance clears and the full renderer issue completes.
...
## Remaining gates
1. After Agent 1 completes exact-SHA validation, clears CPU visual acceptance, completes renderer validation, and merges the validated correction to `master`, merge then-current `origin/master` into `fixes/agent-6` per `master-sync-required.md`.
