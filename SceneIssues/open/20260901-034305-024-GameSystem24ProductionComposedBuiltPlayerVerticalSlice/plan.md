...
7. **After master reconciliation, direct steering from the released player position to the destination NPC is a valid player route.** Falsified by exact-SHA run `34000242054` for product `b0b1e75ba97696a83bc0d2a5658e3fe28e22022f`. Persistent EditMode/PlayMode validation passed and the canonical Kentridge player reached FrontEnd, New Game, gameplay-ready and gameplay-control, then timed out in `MoveToDestination`. The player moved from `(111.5,32.1,82.8)` to `(101.2,22.0,55.9)` but durable frames show the camera pressed against/along dark pub geometry. Production deliberately releases gameplay at the pub interior approach facing the authored exterior doorway; the driver incorrectly projected a direct NPC vector immediately and drove into a wall. Selected fix: while `HasExitedPub` is false, emit ordinary forward player input through the authored public exit, then resume target-relative steering after the semantic exit state becomes true. No teleport, position mutation, completion setter, or authority shortcut is introduced.
...
Current `master` `ef475182b866eabfe8e1d1a39c82bf7810a03f49` was integrated by merge commit `36ecce581846fe6b0e1c021f9980890c279ad3e4`, inheriting the upstream Structures PlayMode harness fix exposed by run `33997426174`. Run `34000242054` then proved the editor test phase green and exposed the first assembled-player movement-route defect; the System24 driver now respects the authored pub exit before steering to the destination NPC.

## Remaining gates

Issue a fresh exact-SHA request on `ci-test/fixes/agent-2` from the post-bookkeeping feature head. Require repository-selected module tests/player validations plus canonical Kentridge integration to pass through every System24 semantic milestone. Inspect durable built-player evidence directly. Only then complete remaining checkboxes, close open -> closed with resolution metadata, merge any newer current master into the feature branch, open/update the PR, enable auto-merge, monitor required `affected`, and verify the closed SceneIssue on `origin/master`.
