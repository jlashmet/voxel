...
- The authoritative merge exposed the expected duplicate Inventory transaction store. Acceptance-driven reconciliation removed that store, retained the richer T09 transaction/result/failure schema, added only the semantic `IInventoryTransactions` convenience seam required by Loot, and migrated Loot regressions to the stateless adapter over `InventoryRuntime`. Current master is an ancestor of the feature branch and `.github/test-request.json` remains absent from the feature ref.
- Next gate: submit the current exact feature source through `ci-test/fixes/agent-2`. Repository-selected automatic validation must prove both Inventory and dependent Loot behavior plus standalone SceneIssue replay before T09-025/T09-032 can be checked or closure can begin.
