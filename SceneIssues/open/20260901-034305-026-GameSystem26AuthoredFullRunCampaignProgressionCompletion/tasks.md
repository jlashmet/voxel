...
- [ ] **T26-043 — Verify shared multiplayer progression/outcome.** **BLOCKED external prerequisite:** System25 remains open on current master and is itself waiting for the System24 production composed built-player boundary; reuse its infrastructure when it lands. Do not create an alternate transport/process harness.
- [ ] **T26-044 — Add canonical built-player full-run scenario.** **BLOCKED external prerequisite:** current Kentridge physical planner intentionally accepts one Kentridge region, while `20260829-020634-000-KentridgeMacroWorldPhysicalRealization` remains open and blocked on the separately owned GPU renderer publication/restoration path. Do not weaken the single-region invariant or fake later regions.
