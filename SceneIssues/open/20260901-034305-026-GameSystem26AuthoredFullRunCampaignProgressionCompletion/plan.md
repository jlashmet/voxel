...
T26-043 first depends on System25's production multiplayer infrastructure. Latest observed System25 source `ae08ee1c97bbfdc8106e525db58147e285a4c54e`, exact request `5bf5078c06f895d637431b7e9ca41fdfac0cc4d9`, run `34202599000`, job `101984620134` failed automatic module validation: the topology player reached topology/baseline/contention/progression/continuity milestones, then timed out waiting for authority `combat-vitality-converged`. Its binding checklist remains incomplete.

System25 completion is an infrastructure prerequisite, **not sufficient T26-043 evidence by itself**. Its current topology scenario asserts shared `progression-converged` but no terminal outcome, and the validation composes `RunningOutcomeQuery.Instance`, whose snapshot is always `GameOutcomeSnapshot.Running()`. After System25 is merged to master, merge that current master into `fixes/agent-8` and add the narrow System26 multiplayer acceptance consumer using the shared process harness plus production Application/Sessions/UTP/campaign/System15 path. Exact-SHA proof must show authority and clients converge on shared Progression and the same immutable terminal GameOutcome; do not duplicate networking or campaign authority.
