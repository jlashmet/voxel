...
- `33492599541`: focused Mountain Dragon acceptance and automatic module validation passed on exact feature source `e511c520...`, but standalone replay failed at the constrained `resolved-49 -> mid-turn` transition. The player accepted `resolved-49` about 1.23 m from its authoritative point under the route-wide 1.25 m tolerance, then remained stationary 4.762 m from `mid-turn` until timeout with streaming settled and no exception.
...
After the second materially different standalone repair still produced a replay timeout, `experiment-017-turn-entry-arrival-radius-minimal-repro.md` isolated the new failure before any third fix. `resolved-49` was accepted 1.23 m early because the generic 1.25 m route radius is intentionally coarse; the next direct steering chord then crossed the constrained switchback inside edge and production collision stopped the player. The shared harness already supports a semantic per-waypoint `arrivalRadius`, so the smallest correction is scene-specific fixture policy: only `resolved-49` now requires 0.35 m proximity before redirecting to `mid-turn`. Production road geometry, motor collision, speed, grade, cut/fill and route points remain unchanged. If exact-source replay still stalls on that transition, experiment 017's falsifier requires inspecting realized terrain/collision before any further fix.
...
## Remaining order
1. Run the exact current feature head with the experiment-017 turn-entry precision repair through only `ci-test/fixes/agent-4`; require focused acceptance + automatic module validation + standalone SceneIssue replay, and inspect `WAYPOINT_REPLAY` grounded/Y evidence and screenshots. Do not replace a queued/running request.
