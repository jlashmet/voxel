...
Exact-source diagnostic run `33718723662` correctly selected feature SHA `8b32f78d599ce24cfe39d2be4fa67d17c5723ef8`, but the requested top-level `VoxelEngine.Tests.PlayMode` serializer never executed because the persistent module sweep fails earlier in unrelated `VoxelEngine.Rendering.Tests.EditMode`. This run therefore provides no route-identity evidence. Experiment 023 records the selection failure. The diagnostic is being moved into the already-selected module-local `Game.Composition.Showcase.Tests.EditMode` assembly so it executes before renderer validation without changing production behavior or shared CI policy.

## Current blockers / independent evidence
- Automatic module validation in runs `33715318543` and `33718723662` is blocked by 15 `VoxelEngine.Rendering.Tests.EditMode` failures outside this assignment; all preceding module assemblies shown by the persistent runner were green.
