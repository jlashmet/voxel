...
`WorldRoadNetworkVoxelCatalogue` proves each presentation segment lowers as a separate bounded corridor definition at the same precedence, while the shared continuous compositor chooses one winner per horizontal column rather than applying segments sequentially. That rules out order-dependent re-filling. Experiment 026 therefore samples the production winner at the stuck p90 footprint and along p90->91 using `ContinuousTerrainCorridorRasteriser.TryChoose` plus `TerrainCorridorRasteriser.TrySample`.

Run `33802313426` did not execute experiment 026 because the diagnostic test assembly referenced `FeatureDefinition.Name` (`FixedString64Bytes`) without an explicit `Unity.Collections` asmdef reference. The persistent log reports only that deterministic compile defect at the diagnostic lines. Commit `0d0999ba277d7383ae40f6748b2ebcce3dfdec7b` adds only the missing test-assembly dependency; production road, raster, route, motor, and scene geometry remain unchanged. The next action is a fresh exact-SHA run of the same winner discriminator, not an infrastructure retry of `33802313426`.

## Current blockers / independent evidence
- The terminal winner discriminator must execute successfully before any further product traversal fix; the previous attempt `33802313426` was blocked by the now-fixed diagnostic test assembly dependency.
...
## Remaining gates
1. Finish the point 88-91 minimal repro by executing experiment 026 and identifying the actual corridor winner/target at the grounded stall; distinguish a shared corridor-composition defect from scene-specific pathological geometry.
