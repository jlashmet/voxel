...
`mountain-dragon-evidence-route.json` is now regenerated from the exact resolved production road. It preserves two south-of-castle normal-movement staging points, an exterior approach capture, then follows every resolved ascent point through the production `CharacterMotor.Step` path. Path base anchors vertical evidence; lower/mid/upper/summit capture points require grounded state and resolver-derived Y offsets; the final point is the same summit approach used by the 90 dm proximity trigger.

## Current integration blocker
Before final visual evidence, current `origin/master` is `ef5240c7b24550dab86d0ed75388d6c99a44d47b`. A mechanical `master -> fixes/agent-4` integration PR (`#202`) against feature head `6098df1000d24ea21fb1c02dcb673f305c929f03` is reported non-mergeable, spanning 126 changed files from concurrent work. The available connector has no conflict-resolving branch-merge operation and local checkout/network access is unavailable. Do not synthesize a merge tree or weaken the requirement. Continue independent current-head route/player validation, but final visual acceptance/closure remains blocked until the real master conflicts can be resolved safely.

## Remaining order
1. Validate the regenerated exact-road waypoint route in the standalone production player on the current feature head; inspect `WAYPOINT_REPLAY` grounded/Y evidence and screenshots. This is independent evidence, not final visual closure while master integration is blocked.
