...
- [ ] Harden only the existing exact GitHub Actions batch successful-bake exit so, after required world disposal, capture restoration, payload/manifest import/save, and success logging, the successful process returns before the 240 s wrapper instead of waiting for Unity's full graceful teardown.
- [ ] Extend the editor-policy regression to prove immediate process termination is requested only by the exact `GITHUB_ACTIONS=true` batch `-executeMethod VoxelEngine.Showcase.Editor.ShowcaseWorldBaker.BakeShowcaseWorld` success path; interactive/local/general test processes must retain normal teardown.
