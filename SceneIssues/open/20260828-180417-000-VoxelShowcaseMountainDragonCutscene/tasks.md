...
- [ ] Isolate the production `CharacterMotor` late-stall blocker through the exact player-capsule collision path. Determine whether authoritative voxel `IsSolidAabb` or semantic `VegetationComposition.TreeDamage.OverlapsWoodAabb` blocks the sweep, including the grounded step-up path if needed, before changing route/motor/terrain/vegetation policy; add/update a reusable regression at the responsible ownership boundary.
