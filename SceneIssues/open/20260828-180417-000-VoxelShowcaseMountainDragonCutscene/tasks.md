...
- [ ] Correct the demonstrated centered-lane headroom regression from exact visual-final run `33343768361`: each shell-following segment's carve must remain exactly the configured 16-voxel traversal width instead of inheriting Z retreat through `MountainPathSegmentGeometry.Depth` (observed width 33). Rerun the exact focused acceptance before another visual-final request.
