...
- [ ] Repair the scoped-builder call through the existing `VoxelEngine.Composition.FeatureRegionBuild` bridge so Showcase composition does not cross the Structures.Runtime type boundary; preserve the bridge's default full-runtime scope and existing `CompleteFeatureBuild` bookkeeping.
