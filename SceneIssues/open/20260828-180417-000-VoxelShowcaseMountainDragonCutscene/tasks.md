...
- [ ] Isolate a minimal realized terrain-corridor/collision mismatch around current resolved points 88-91 before any further route-control, motor/tolerance, grade/cut-fill, or summit-placement change. Run `33754305666` proves segment 90->91 needs only 3-10 dm of cut while its emitted corridor allows 42 dm and clears 24 dm above; insufficient cut depth is ruled out. Next discriminator: identify same-precedence overlapping corridor definition/order at the sharp terminal turn and whether a neighboring segment rebuilds a higher surface at the grounded stall.
...
- [x] Run `33754305666` executed and passed `CurrentProductionTerminalCorridorSerializesForCollisionIsolation`. It measured analytic terrain over segment 90->91 at +3/+5/+5/+6/+7/+8/+9/+9/+10 dm above road target while emitted `EmitTerrainCorridor` allows 42 dm cut and 24 dm clear-above. Overall workflow later failed unrelated GPU renderer oracles, so the targeted mountain evidence is retained and the run is not retried as infrastructure.
