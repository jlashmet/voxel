...
1. **A production Vitality Runtime is required before duplicate Kentridge HP authority can be removed.** Current-master inspection confirms the Runtime is absent, so T01-002/010/011/012/014/015 and their assembled follow-ons remain blocked on that prerequisite.
...
- Do not patch renderer teardown, create a fake Vitality implementation, or take ownership of System 02 Runtime.
- When the real Vitality Runtime is published, merge current master, inject it in Kentridge composition, bind production CharacterIds, remove production use of the parameterless Combat health path, and keep Encounter/game-result policy in composition.
...
- Real Vitality Runtime publication and Kentridge production cutover.
