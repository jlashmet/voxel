...
- T006 implementation is on feature SHA `c02cf6dbda810cc01dc1a32970844cd0eb494e62`: `IWorldVisibilitySource` exposes descriptor-only deterministic queries, while `WorldVisibilityManifest` indexes integer sectors, deduplicates cross-sector structures, orders results by stable structure key, and supports stable replacement/removal without voxel/storage/render dependencies. `WorldVisibilityManifestTests` covers cross-sector queries, insertion-order independence, replacement/removal, negative sectors, and querying a never-resident preplanned descriptor. Exact-head targeted CI run `33382040502` is in progress via `ci-test/fixes/agent-7`; do not replace it.
