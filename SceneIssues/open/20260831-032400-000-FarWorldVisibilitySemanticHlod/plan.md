...
- T009 engine rendering boundary is now present in `Assets/VoxelEngine/Rendering/Api/FarWorldRendering.cs`. It defines stable render-ready structure identity/transform/bounds, semantic proxy/style keys, tier/visual flags, and `IFarStructureRenderer` without depending on Game/WorldBuilder types. The implementation follows the existing Rendering.Api batch-renderer shape and keeps distance/semantic selection policy outside the renderer. It remains unchecked until exact-current-SHA compilation/behavioral validation.
- T010 composition adapter is now present in `Assets/Game/Composition/Showcase/SceneRuntime/ShowcaseFarStructureSource.cs`. It queries only `IWorldVisibilitySource`, injects tier selection and ground-height policy, converts deterministic decimetre bounds to render instances, preserves stable structure IDs, and never requests voxel residency. `ShowcaseFarStructureSourceTests` is an independent fake-source fixture covering stable identity, dimensions/elevation, landmark flags, and cull omission. T010 remains unchecked until exact-current-SHA compilation/behavioral validation.
