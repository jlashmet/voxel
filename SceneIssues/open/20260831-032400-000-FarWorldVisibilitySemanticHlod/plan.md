...
- Earlier targeted CI run `33378953354` passed the requested focused coverage test on feature SHA `7797ce0de134fc2e1c246000e3249f4ae0d6d582` but failed derived module validation without a recoverable module log in the artifact. Do not classify that module failure as infrastructure or guess at a product fix.
- T003 is now independently actionable: the existing diagnostic surface is `CompactFpsOverlay`, and `VoxelFarTerrain` already exposes enough coverage math to add a read-only coverage diagnostic without creating a new scene object. Implementation is pending because repository writes require whole-file replacement through the connector for the large `VoxelFarTerrain.cs`; acceptance is unchanged.
- The earlier T004/T006 implementation does **not** satisfy the tracker contract and must not be marked complete merely because its focused tests pass. Current `StructureFarPresentation` contains prebuilt silhouette geometry instead of the required compact semantic descriptor (stable structure key, footprint/bounds/height/facing, archetype/material/cluster keys, semantic visibility class, deterministic revision). Current `IWorldVisibilitySource`/`WorldVisibilityManifest` is a snapshot-array API rather than the required deterministic sector index with bounds queries plus stable replacement/removal. Rewrite these contracts to the task wording before checking T004/T006.
- T006's first exact-head request `33382040502` failed compilation because `Game.WorldBuilder.Api` lacked the transitive Core reference required by the current descriptor. Feature commit `1a9f86459aeaadc1fad6282f2a1c65d73a95b559` fixed that assembly dependency; replacement run `33383221825` passed the focused test and derived module validation. This validates compilation only, not the mismatched T006 acceptance contract.
- T005/T007 are no longer blocked on locating the Showcase handoff: `ShowcaseWorld.QueueLandmarks()` plans the castle before `_castleRegions` are populated and is the correct registration boundary once `CastlePlan` carries the T005 descriptor. The concrete castle planning type still needs to be located/inspected before changing it.
...
- Sole targeted-CI transport remains `ci-test/fixes/agent-7`; `.github/test-request.json` is not on the feature branch.
- CI request commit `46c3eb39dd074fa4b8f6492028a4647b6d3b8a9a` requests `VoxelEngine.Tests.EditMode.StructureFarPresentationTests` against feature source SHA `1a9f86459aeaadc1fad6282f2a1c65d73a95b559`. Run `33386708568` is queued as of the latest poll. Never replace or rewrite the CI transport while that run is queued/running.
- The execution container has no authenticated Git checkout and no outbound DNS. Repository reads/writes are through the connected GitHub API. Small/medium files can be edited safely as whole-file replacements; large-file changes should avoid speculative reconstruction and must preserve blast radius.
