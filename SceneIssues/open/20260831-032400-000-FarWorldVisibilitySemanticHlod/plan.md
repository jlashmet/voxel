...
- T026 wires `VoxelShowcase` to the canonical feature-presentation source, shared semantic selection/state adapter, and `ProceduralFarFeatureRenderer`; runtime queries the scene camera without requesting distant voxel residency and owns renderer lifetime with the Showcase.
- Exact-SHA run `33736795821` failed before the focused T026 regression because `ShowcaseFarFeatureRuntime.cs` compiled in the nested `VoxelEngine.Showcase` assembly, whose asmdef referenced `VoxelEngine.Rendering.Api` but not `VoxelEngine.Rendering.Runtime`. The top-level Showcase asmdef is not inherited across the nested assembly boundary. This is the demonstrated compile root cause, not the prior GPU baseline blocker.
- Narrow fix: add `VoxelEngine.Rendering.Runtime` to `Assets/Game/Composition/Showcase/SceneRuntime/VoxelEngine.Showcase.asmdef`; no renderer API or implementation change is required. Revalidate this exact feature head on the sole `ci-test/fixes/agent-7` transport before checking T026.
