...
T020 now has an explicit `VisibleVegetationBatchAdapter` between deterministic visibility records and the existing GPU-instanced renderer. The adapter updates only the queried visible subset, reuses its presentation scratch list, and `ProceduralVegetationBatchRenderer` retains per-kind batch lists across visibility-window changes instead of reconstructing them. Focused regressions cover invisible-sector exclusion and stable batch-key reuse; exact-head CI is still required before checking T020 complete.
...
Run `33433500592` is green for the far-terrain detail-shader validation source and proves the new shader path compiles in targeted CI. It does not replace the required 0.5/1/3/6/10/12 km visual comparison.

Run `33439521492` failed during Unity compilation before the requested vegetation regression could execute. Artifact inspection isolated the product cause to `FarTerrainMaterialFamilyTests` directly importing `VoxelEngine.Composition.Api`, an assembly boundary that the EditMode test compilation does not expose. The regression was retained but changed to exercise `VoxelFarTerrain.ResolveFarSurfaceMaterial` through its runtime method signature/reflection, avoiding a new test-only assembly dependency. Replacement run `33441157048` targets that corrected parity regression and is queued; do not replace it while queued/running.
