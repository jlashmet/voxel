...
- [ ] **T25-003 — Add exact-SHA/build identity assertion.** Harness and process sides are implemented: each launch/relaunch automatically validates process-reported source SHA + executable SHA-256; Kentridge validation reports the authoritative source SHA while independently hashing its actual running macOS executable, and the focused encounter-realization target now runs through `mode: multiProcess` with a real semantic milestone. Exact feature `8241f75845a3136676b91c7e612c48d5b3ceab92` is queued as request `c8993bf9615705ccadf69a2904d871dd679f2fb7`; check this task only after that exact built-player proof passes.
