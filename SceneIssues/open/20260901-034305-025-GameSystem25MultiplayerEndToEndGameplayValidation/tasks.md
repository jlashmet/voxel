...
- [ ] **T25-004 — Isolate writable process state.** Correctness audit reopened this task: relaunch attempts had been reusing HOME/temp/config/cache as well as the intended durable state root, so a nominally fresh reconnect process could inherit ephemeral state. The runner now attempt-scopes HOME/temp/config/cache while preserving only `VOXEL_VALIDATION_STATE_ROOT` across attempts; the regression asserts attempts 1/2 share durable state but not ephemeral roots. Exact-SHA validation of the corrected implementation remains required before checkoff.
...
- [ ] **T25-008 — Prevent stale milestone reuse across sequential assertions.** Correctness audit found that a second wait for the same semantic milestone could re-read and accept the first matching log event, invalidating repeated convergence/reconnect proof. The runner now tracks a monotonic per-attempt milestone cursor, preserves repeated identical-name events distinctly in durable history, and fails closed if the milestone stream shrinks. A focused Python regression requires two `state-converged` waits to consume revisions 10 then 11; exact-SHA validation remains required before checkoff.

## Production topology / entry

> **External prerequisite:** on `origin/master` `939e9a6f744313d93992b0479d5f6140d774ef42`, `20260901-034305-024-GameSystem24ProductionComposedBuiltPlayerVerticalSlice` is still open and T24-032 remains unchecked. Agent-2 is actively advancing that prerequisite; System25 must not copy or substitute its unmerged production composition/diagnostic path. Continue independent validation-infrastructure work only.
