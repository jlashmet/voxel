...
- [ ] **T25-010A — Complete Application's production joined-party startup.** Initial implementation `8b95feaf7d849bc6a37b4d5a40a4e84b7e8c331a`; latest correction `cb54eeec77a7178770e4a4c3260276cd9f26c520`. Wait for Active plus matching connected/gameplay-ready local membership before one local graph startup. Do not send client Start, grant readiness, or retry failed composition every frame; preserve leave/rejoin reset. Inspection found the earlier fixture masked production Orchestration's strict pre-Prepare readiness requirement. Replaced its substitute state machine with the real `GameSessionOrchestrator`; 26 authored NUnit cases now cover pre/post-start readiness and disconnected/recovery/left states alongside existing cases. The owned Application player scene now runs `ApplicationJoinedPartyValidation` through production Application/Orchestration, replacing the previous joined-start stub proof; synchronization-wait, single startup, leave and fresh rejoin are required. **Execution still pending:** original request `920f0e4e4883d2c8abaf77877c1f8e55c8cd4df3` for source `8b95feaf7d849bc6a37b4d5a40a4e84b7e8c331a`, run `34002524305`, job `101403824713` remains queued at last inspection. Preserve it while queued/running. It does not cover newer source `cb54eeec77a7178770e4a4c3260276cd9f26c520`, which needs a later exact request. Keep unchecked until behavioral and module-player gates pass. These boundary checks do not replace separate-process topology evidence.
