...
- [ ] **T25-010A — Complete Application's production joined-party startup.** Implementation `8b95feaf7d849bc6a37b4d5a40a4e84b7e8c331a` observes matching session/local-member state and consumes one local startup without client Start/readiness grants. Follow-up `cb54eeec77a7178770e4a4c3260276cd9f26c520` requires connected/local synchronization before Prepare and uses real production Orchestration in regressions and owned Application player validation. **Unvalidated:** exact request `920f0e4e4883d2c8abaf77877c1f8e55c8cd4df3`, run `34002524305`, job `101403824713` remains queued and tests only the earlier source. Do not replace while queued/running. Keep unchecked until latest behavioral and module-player gates pass. This is not separate-process topology proof.
- [ ] **T25-010B — Carry session admission through production UTP.** Required by T25-010/011: add explicit bounded client admission request/server reply routing over existing EVENT, preserving transport-owned sender identity and rejecting malformed/direction-mismatched/unhandled packets. Net does not grant membership or gameplay readiness. Extend existing Net runtimes; add owned `Assets/VoxelEngine/Net/Tests/EditMode` regressions and create its `Validation/SessionAdmissionTransportValidation.unity` plus paired scenario using real runtime loopback. Module-level transport evidence does not satisfy production Sessions/provider topology or full multiplayer acceptance.
