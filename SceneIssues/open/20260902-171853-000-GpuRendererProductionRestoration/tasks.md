...
- [x] **TGPU-006 — Stop speculative fixes after repeated failure and isolate the root cause.** Two materially different interventions reproduced the identical Planar/Smooth parity symptom: run `33667605313` synchronized the CPU/GPU water-mask contract and still failed, and run `33668767375` additionally bound `_SolidWaterMaterialMask=0` directly on the production `ComputeShader` instance yet still returned GPU `-0.14` with material/style correct and the authoritative-solid bit absent. The include-only classifier probe passed on retry of run `33668009978`; full-mesher local-call probe run `33669435649` returned material 1, solid=true, and density +0.5; run `33670311041` falsified SRV/UAV aliasing because write-only `CSSampleDensity` still returned -0.14; run `33671055825` proved the 64-thread coordinate and dense brick lookup are correct. Exact-SHA run `33671401799` then isolated the Metal compiler/codegen hazard: the same `SampleField` call returns +0.5 when first assigned to a local, but the production expression `_DensityWrite[index] = SampleField(... out locals ...)` returns -0.14 and loses authoritative occupancy. The production single and batch sampling kernels use this same hazardous expression shape.
