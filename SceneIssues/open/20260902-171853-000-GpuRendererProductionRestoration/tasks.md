...
- [ ] **TGPU-019CPU4F — Prove the bounded canonical far-frustum correction.** Required discovered work: the production mountain emits a frustum, but the old adapter loses centre/direction/radii and the renderer substitutes a box. Source `da3f5be...`, fail-before request `6ddc727...`, run `33999899224`, artifact `9979637933`: terminal failure is classified; exactly eight intended silhouette cases fail (expected radius 11.5 +/- 1.25, actual 24.5), while 289 other Rendering tests pass. Candidate `a164456...` preserves resolved cap geometry and uses the existing renderer plus module-owned player scene. Pass-after request `fc6c3320d9b986b8d2401fcae0a17de80d286691`, run `34003412217`, directly tests feature `e4e2f997...` and is queued. Do not replace it. Require passing exact-SHA tests/module players and inspected VoxelShowcase replay before checking this task. Other primitive mismatches and full CPU visual acceptance remain open.
