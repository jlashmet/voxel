...
- [ ] **TGPU-005 — Discriminate centre-read vs smooth-field failure.** In the same full-mesher/buffer-binding environment compare raw `ReadMaterial`, planar early-return `SampleField`, and smooth `SampleField`. Record H1/H2 result in `plan.md` before changing production code.
- [ ] **TGPU-006 — Stop speculative fixes after repeated failure.** If two materially different fixes hit the same parity symptom, isolate a minimal shader/buffer repro or proven root cause before another production change.
