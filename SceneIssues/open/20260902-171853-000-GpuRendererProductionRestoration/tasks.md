...
- [ ] **TGPU-019CPU4F — Prove the bounded canonical far-frustum correction.** Required discovered work: the production mountain emits a frustum, but the old adapter loses centre/direction/radii and the renderer substitutes a box. Regression source `da3f5be...`; fail-before request `6ddc727...`, run `33999899224`, currently queued. Candidate repair `a164456...` preserves resolved cap geometry and uses the existing renderer plus module-owned player scene. Require terminal fail-before classification, exact-SHA pass-after tests and inspected VoxelShowcase replay before checking this task. Other primitive mismatches and full CPU visual acceptance remain open.
