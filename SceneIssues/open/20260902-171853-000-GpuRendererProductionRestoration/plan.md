...
The historical regression window is now three commits wide. Passing `5716e56` is followed by `b4de1b576dfb06821ada42b6094bf9cbe7c9c31f` (`Resolve persistent mirror bricks on GPU`), which introduces the persistent GPU directory lookup into `VoxelBrickDensity.hlsl`; `58811d6aa4c6a2a2cbc330bbdc9747f49e9fff0e` changes only C# staging/context; and failing `44433afcbd12a23c0037058dc27a2df1b2835002` changes only the persistent lookup header/loop in the density include. Dense-mode oracle data are correct and never execute the persistent branch, so the remaining root-cause work is to identify which persistent-lookup construct changes Metal code generation for `SampleField` despite being untaken at runtime. The next discriminator must isolate that construct rather than make another speculative production semantic change.
...
## Current root-cause boundary / next discriminator

The introducing change is no longer attributed to the UAV assignment itself. The latest evidence localizes the regression to persistent-directory lookup code added to `VoxelBrickDensity.hlsl` between passing `5716e56` and failing `44433af`, even though dense-mode tests do not take that branch. Candidate compiler-shaping constructs include the nested `TryPersistentBrickEntry(..., out uint entry)` call and its dynamic probe loop. Isolate those constructs with a minimal semantic-preserving discriminator before changing production behavior. Preserve dense and persistent lookup semantics, water/material classification, source-step rules, and CPU authority while doing so.
