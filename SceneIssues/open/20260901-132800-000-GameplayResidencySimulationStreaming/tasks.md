...
- [x] **R113 — Run focused deterministic/module tests.** Preserved exact request `1ca35bbb8f5d4a08cb69ad44488971e4937fc4aa` executed all 18 repository-selected affected EditMode assemblies plus the focused Residency regression with zero failures/skips/inconclusive tests.
...
- [x] **R114B — Repair invalid module-local player scenarios.** Preserved module validation stopped before Residency because Residency/Streaming requested 6s/5s while the shared harness requires at least 10s. Raise both module scenarios to 10s without changing runtime behavior.
- [x] **R114C — Reference Input System from the actual owning Kentridge scene-runtime assembly.** Request `219c6c85e71c05a97a0dda6724811c53b0897e1c` failed compilation because the migrated reader belongs to `Game.Kentridge.PlayableSlice`, not the parent composition asmdef. Move the dependency to the nested owner and remove the unused parent reference.
- [x] **R115A — Make coordinator teardown obey the R32 pin-release invariant.** Final review found `Dispose()` could release physical leases without quiescing Detailed owner adapters. Teardown now drives normal demotion first, keeps the pin if demotion is pending/failed, and has focused ordering/retry regressions.
