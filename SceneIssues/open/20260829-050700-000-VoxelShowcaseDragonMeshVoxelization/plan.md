...
Direct inspection of those nine Dragon captures exposed a second validation-composition defect: the source camera was tagged `MainCamera`, so production voxel rendering scheduled around the presentation side ~110 m away. The right authoritative viewport was blank in most directions and, in the side view aligned with the separation axis, showed only the tiny distant presentation Dragon. This evidence is not production-quality and must not be accepted. Commit `17f9c44407977b8851a161c7b43c4cbdd374e137` makes the voxel camera `MainCamera` and shifts the first semantic-view transition to a relative midpoint between screenshot intervals; commit `449bacd67be19f5432447716f4efd2849b892cd7` extends the scenario to 37 s while retaining `minimumFrames=10`, so each of the ten required semantic views can receive a distinct durable frame. These are fixture/evidence repairs only; production voxel/storage behavior is unchanged.
...
## Next gates
1. Re-run the repaired module fixture on the current exact feature SHA through `ci-test/fixes/agent-1`; inspect all ten built-player captures directly and require matched readable source/voxel framing plus required logs/no exceptions.
