...
- [ ] Vendor the deterministic OBJ source/reconstructable archive into this repository without changing geometry. Exact payload is recovered and reverified locally; transfer is in progress with contiguous repository parts `part00`-`part11` committed. Remaining exact parts must be committed and final reconstruction/hash verification rerun before checking this item.
...
- [ ] Add an independent non-dragon regression for explicit nearly-closed `VoxelShellFill`: open/non-manifold source topology may fill only when conservative rasterization encloses voxel-space cells; existing `Reject` and `SurfaceOnly` semantics must remain unchanged.
...
- [ ] Add explicit reusable nearly-closed voxel-shell fill policy to the shared importer, preserving `Reject` and `SurfaceOnly`; use it only through explicit configuration after the demonstrated source-topology defect.
- [ ] Add source-specific bake configuration and generate/commit the baked dragon artifact within structure bounds X/Z<=127 and Y<=511. Configuration exists, but current `Reject` topology setting must be replaced only after the reusable explicit voxel-shell policy/regression is implemented; then generate from the exact reconstructed source.
