...
- [ ] Vendor the deterministic OBJ source/reconstructable archive into this repository without changing geometry. Runs `33362323833`, `33364246307`, and `33364871767` prove respectively wrong segmented payload identity, mixed whole+segmented rejection, and a 319,999-character whole-manifest transfer. History-side isolation proves original/remaining whole `part13` is 19,999 bytes, the first repair was 9,999 bytes, the second restored the same malformed 19,999-byte blob, and no known-good whole exists. The segmented form is 4,999 + 10,000 + 5,001 but already failed the pinned gzip identity. Keep unchecked until the bounded missing-character repro identifies a unique candidate against both pinned hashes and the original exact-source test passes.
...
- [x] Isolate repeated source-transfer acceptance failures to the historical `part13` transport: initial whole 19,999 bytes; first whole repair 9,999; second repair same 19,999; historical 5k probe 4,999; staged 5,001-byte `part13.01fix` is the later tail, not the missing single character. No further source mutation until the minimal repro proves an exact correction.
- [ ] Run `MountainDragonSourceArchiveTests.DiagnosePart13TransferLoss_MissingProbeBoundaryCharacterHasUniquePinnedIdentity` on an exact feature SHA through `ci-test/fixes/agent-1`. It may search only the demonstrated 4,999-character probe boundary first; require a unique character matching pinned gzip + decompressed OBJ identities. If this discriminator fails, broaden root-cause isolation before another fix.
...
- [ ] Add source-specific bake configuration and generate/commit the baked dragon artifact within structure bounds X/Z<=127 and Y<=511. Dragon composition now explicitly opts into `VoxelShellFill`; exact artifact generation/validation remains required.
