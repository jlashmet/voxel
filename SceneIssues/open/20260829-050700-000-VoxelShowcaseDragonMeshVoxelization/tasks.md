...
- [ ] Instantiate the decoded baked dragon through normal `ShowcaseWorld.PlaceBakedMeshStructure` / WorldBuilder voxel authoring so rendering, collision, edit, and destruction share canonical storage. `MountainDragonBakedArtifactTests.CheckedInBake_PlacesThroughNormalShowcaseWorldAuthoringPath` passes in exact-parent runs, but built module validation still needs to complete replay/rendering. Runs `33476063393` and `33477581134` isolated the fixture bypassing production brick-budget sizing; commit `89bf1837fba3c555bf6fab873c70c6bd5839a899` now uses the production tier-byte authority. Run `33479919740` did not execute this runtime path because `Game.Composition.Showcase.asmdef` omitted the fixture's existing `VoxelEngine.Tiering.Api` dependency; commit `092daab402eee78b5b944fd8bf16129afc18aa11` repairs only that compile boundary and the runtime gate remains pending.
...
- [ ] Add labeled matched `Mesh -> Voxels` comparison area with identical effective pose/scale/orientation/ground/lighting; source mesh is presentation-only and has no gameplay/collider authority. Implementation exists in the module fixture; built-player validation and direct capture inspection remain required before checking this item.
...
- [ ] Integrate durable capture support for front, side, rear, front 3/4, rear 3/4, top/elevated 3/4, head/horns, wing, feet/claws, and tail. Semantic ten-view capture contract exists; successful real built-player evidence remains required.
- [ ] Emit/record source triangle count, voxel resolution, authored voxel count, sparse brick/chunk count, voxelization duration, serialized size, resident/runtime placement/build cost. Bake metrics already record source triangles/resolution/cells/bricks/voxelization/serialized bytes; module fixture logs placement time, tier budget, brick capacity, and runtime allocated/reserved bytes after successful placement, but exact-SHA built evidence remains required.
...
- [ ] Source and voxel exhibit use the same effective transform/pose; material/color acceptance is explicitly correct for STL with no standard material regions. Implementation is present; built capture inspection remains required.
