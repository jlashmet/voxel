...
- [ ] Vendor the deterministic OBJ source/reconstructable archive into this repository without changing geometry. Runs `33362323833`, `33364246307`, and `33364871767` prove respectively wrong segmented payload identity, mixed whole+segmented rejection, and a 319,999-character whole-manifest transfer. Exact-SHA diagnostic run `33377081586` now proves known repository transfer variants are contradictory: the current 4,999-character prefix is internally consistent with current `part13` but does not equal historical `part13.00`, while the independently staged 5,001-character tail differs from the current 5,000-character window by one deletion. Repairing that demonstrated deletion still does not recover the pinned gzip/OBJ identity. Authoritative source bytes are therefore unavailable from known repository history; keep unchecked until the exact source/archive is supplied or otherwise recovered and the original exact-source test passes.
...
- [ ] Add dragon-specific production regression proving required anatomical regions are non-empty/spatially plausible through produced bake data, not source-string/count-only assertions. Blocked on exact authoritative source/bake artifact; continue independent implementation work.
...
- [ ] Add source-specific bake configuration and generate/commit the baked dragon artifact within structure bounds X/Z<=127 and Y<=511. Dragon composition now explicitly opts into `VoxelShellFill`; exact artifact generation/validation is blocked on authoritative source recovery.
