...
## Next discriminator
Use only `ci-test/fixes/agent-5` for one exact-SHA focused PlayMode + exact-scene player request after the current head is reviewed. Focused class must be green; player must pass all three traversals/negative contracts and emit eight frames. Inspect every full-resolution frame directly. Only `production-quality` passes.
...
## Remaining gates
- Green exact-SHA focused + built-player run.
