...
## Next discriminator
Use only `ci-test/fixes/agent-5` for one exact-SHA focused PlayMode + exact-scene player request from the current feature head, with a replay window long enough to finish all eight structural frames. Focused class must stay green; player must reacquire the structural district, pass all three traversals/negative contracts, and emit eight frames. Inspect every full-resolution frame directly. Only `production-quality` passes; any bridge/cliff terrain occlusion keeps the issue open.
...
- Valley-side camera correction is audit/evidence policy only and changes no authored structural content.
...
## Remaining gates
- Green exact-SHA focused + built-player run, including structural residency re-entry and all eight durable source frames.
