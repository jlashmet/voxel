...
`KentridgeUrbanFabricSpacingPlayModeTests.ProductionAnonymousFrontagesLeavePedestrianClearanceBetweenHouses` builds `KentridgeFrontageAlignedUrbanFabricCatalogue`, the exact production stage used by the combined Kentridge catalogue, and checks actual placements/footprints for >=20 dm lateral clearance. The final named-plot reservation may only remove sites, so this is conservative. First CI failed before execution on a test-only `CS8156`; the regression now copies native-array values to locals before comparison. Production code is unchanged by that correction.

Cost improves: anonymous definitions/placements fall 42→26 (~38% fewer), with unchanged per-building primitive budgets. Remaining gates: final exact-SHA PlayMode CI, saved-pose replay/artifact inspection, `verification-final.png`, final issue bookkeeping, closure, and non-force master integration.
