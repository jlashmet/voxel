...
- Behavioral EditMode coverage executes production town recipes, distinct results, semantic Showcase recipes, and supplements with a scene-source backend guard.
- Final request from source `406a30415537634d9b122287a516e952e5ef1fda`, run `33201049016`: real macOS player built/launched, but production Kentridge acceptance failed because `first-destination` was unreachable from `starting-pub` under `NormalParty`.
- Discriminator confirmed stale traversal facts: organic Kentridge has zero legacy `Streets` and inferred `Routes`, while `SettlementStreetTraversalFacts` read only streets and rejected `SiteAccessKind.Route`.
- Product fix commit `5c87db87921c4a8b25dbaaf0b9866cecb98ce375`: shared traversal graphs streets plus arbitrary inferred route segments, splits exact integer intersections/endpoints, and binds access by declared path id. Regression keeps Kentridge at zero streets and requires pub-to-every-site reachability through production traversal.
...
- No other capture or feature-branch CI request file is changed. Fresh assignment authorizes one final request from the corrected feature head; focused regression plus exact-scene built-player success are the remaining promotion gates.
