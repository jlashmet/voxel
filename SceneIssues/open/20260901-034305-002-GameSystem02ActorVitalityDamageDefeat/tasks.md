...
- [ ] **T02-015 — Migrate combat health.** Adapt existing Combat participant health/alive access to Vitality API; remove duplicate authoritative combat health after behavior parity.
  - **BLOCKED pending System 01 publication:** T01-003 is now implemented and checked on `fixes/agent-3` head `933fedfa09c7f6fe4f8536eb3618ba89cb59dfc4`; its `Game.Combat.Api.CombatParticipant.FromCharacter(CharacterId, CombatTeam)` supplies the required semantic Character-backed participant identity. However current `origin/master` is `b274014ae201153c816c981a1092ad8b0d0a7539` and does not yet contain that contract. Agent-9 must not copy/cherry-pick unpublished work from another assignment or invent its own `CombatParticipantId` -> `CharacterId` policy. Resume immediately after the System 01 binding lands on master/coordinator flow.
