...
- [ ] **T02-025 — Run automatic module tests and dependent Combat tests.** Do not manually enumerate CI tests.
  ...
  - Prior blocker isolated: after the successful automatic player harness, renderer teardown threw `NullReferenceException` in `GpuSurfaceMirrorCoordinator.DetachPageArena` via `VoxelSurfaceScheduler.Dispose`, then Mono crashed during shutdown.
  - **Unblocked:** validated GPU renderer restoration landed on `origin/master` at `f5593cc1236ba3963fc5713a11df35292628e97d` and was merged into this branch as `8946d54e4b64820b47fb4a55ca0be98dee543c3b`. Resume T02-025 with a new exact-SHA automatic module/player and standalone SceneIssue CI gate before checking this task.
