...
Origin refresh on 2026-09-04 confirmed master `d08612dfe2f4a99aff34897717569744565bc642`; the feature branch merged that master at `25e9d072ee2b6f923a17260dcc9a9a81361d25df` with no Audio/Kentridge conflicts and is no longer behind master. Compatibility exact-SHA request `cb3c694eb3f24219d423aae64513b2dc01e9a77e` (run `33888505678`) targets that synchronized production head and is currently queued; leave it untouched until completion.

System 23 is still in `SceneIssues/open`, repository search finds no `IUserPreferencesStore`, and System23 tasks T23-007 (define the preference contract) and T23-017 (persist/apply audio settings) remain unchecked. The synchronized master therefore does not supply or supersede the missing preference seam. T21-016 remains the only external implementation blocker. Keep this SceneIssue open; do not invent Audio-owned settings authority. When system 23 lands, merge then-current master, bind its real preference seam to `AudioPresentationRuntime.ApplyMix`, run exact-SHA validation on that integrated production head, complete T21-016/T21-032, then close and promote by PR + auto-merge.
