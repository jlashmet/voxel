...
T21-016 remains the only external implementation blocker: current master is still `aa61895f28d70f35c67d07db6a4fa93beee635eb` and does not contain the system 23 preference API. Keep this SceneIssue open; do not invent Audio-owned settings authority. When system 23 lands, merge current master, bind its real preference seam to `AudioPresentationRuntime.ApplyMix`, re-run exact-SHA validation, complete T21-016/T21-032, then close and promote by PR + auto-merge.
