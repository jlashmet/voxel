...
- [x] **T17-001 — Inventory current HUD/debug UI.** Find Kentridge/prototype health labels, prompts, combat/encounter indicators, objective text, connection/loading status and hardcoded key strings.
  - Evidence: `KentridgePlayableSlice` owned prototype `OnGUI` state/loading labels, hardcoded `WASD`/`E`/`Shift`/`Space`/`F10`/`Esc` strings, and locally scanned campaign NPC positions to produce `E talk to ...`; `HandleKeys` independently polled `KeyCode.E` for the same interaction.
...
- [x] **T17-018 — Replace prototype/Kentridge hardcoded GUI.** Production composition uses Hud module; remove duplicate labels/prompts after parity.
  - Evidence: Kentridge now installs `KentridgeGameplayHudInstaller`, which projects the System14-owned character/vitality/encounter/input-context services through the reusable `HudSnapshotProjector` and real `GameplayHudPresenter`. `KentridgePlayableSlice` no longer renders the prototype gameplay state/prompt panel and routes Interact/Cancel/Sprint/Jump through the same semantic `UnityInputBindingService` used for HUD labels. Exact feature SHA `3775c5816af10e1d5d4b6253898068ac00de3a5a`, request `4ca3f519e15e5470858e379c3b8f3fbdf2216f68`, workflow `33869572240` passed module validation, Hud player validation, Kentridge module player validation, canonical full-app Kentridge integration, and standalone SceneIssue replay.
...
- [x] **T17-025 — Module-local built-player visual validation.** Use shared validation harness and semantic milestones; no bespoke screenshot driver.
  - Exact-SHA workflows `33864869873` and `33869572240`: `Assets/Game/Hud/Validation/HudSemanticPresentationValidation.unity` passed through the real `HudSnapshotProjector`, production `GameplayHudPresenter`, and real `UnityInputBindingService`; player log emitted `HUD_SEMANTIC_PRESENTATION_VALIDATION PASS vitality=73/100 prompt=E combat=true readiness=GameplayReady objective=Reach the gate`. Direct inspection of durable settled screenshots confirms a crisp readable hierarchy, consistent panel alignment, semantic state accents, immediate vitality/prompt readability, and no placeholder or validation-only presentation path. The canonical Kentridge full-app capture also visibly contains the same production HUD surface.
...
- [x] **T17-030 — Remove hardcoded physical prompt strings and duplicate HUD truth.** Repository search for `Press E`/key-name equivalents and HUD-owned gameplay values.
  - Evidence: the shipped Kentridge gameplay surface no longer renders `WASD move • mouse look • E interact • Shift sprint • Space jump`, `E talk to ...`, or the duplicate state box. Interact/Cancel/Sprint/Jump now consume semantic actions from `UnityInputBindingService`; F10 remains a developer rescue action rather than HUD/gameplay prompt truth. Green exact-SHA workflow `33869572240` proves the cleaned production scene compiles and runs through both Kentridge module and full-application player gates.
