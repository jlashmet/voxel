...
3. **Evidence staging failure — supported.** Exact request `f0657d7a…` was workflow-green but its built-player artifact had no hero-arch frame or `ARCH_EVIDENCE` log. The launcher uses `-voxel-scene-issue`; the harness parsed only `-voxelIssue`.
...
- Ticket-gated `KentridgeLandmarkEvidenceHarness` accepts the canonical scene-issue switch (plus legacy compatibility), stages the semantic Warehouse approach, and uses the normal eye-height camera.
