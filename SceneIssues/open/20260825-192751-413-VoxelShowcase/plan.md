...
- **GPU mirror publication performs synchronous driver work — supported.** Job `98980211076` failed moving p95 at 91.445 ms. Worker max `46.408 ms` mapped to solid admission `46.409 ms`, and `98.201 ms` mapped to `98.203 ms`, while arena geometry upload was ~0.345 ms. Source issued three immediate `ComputeBuffer.SetData` payload writes plus metadata for each newly admitted mixed brick.
