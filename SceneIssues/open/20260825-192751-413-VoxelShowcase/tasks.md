...
- [ ] **Remove obsolete minimum-face GPU fallback.** `ExactSnapshotClassificationJob.LowBoundaryTouchesNoSolidHalo` was added before the GPU mesher learned the negative ownership shell. Commit `9275602c3610079a2966cd022b1a3f2fb13d8b62` later changed GPU regular-cell count/write to `(CellsPerAxis + 1)^3`, maps cells to `-1..CellsPerAxis-1`, and applies the same selected-inside ownership rule. The production fallback has now been removed; replace its old regression with a GPU negative-shell ownership oracle before marking this complete.
...
- [ ] **Fix CPU topology/attribute oracles for negative-shell lanes.** `TransvoxelTopologyJob` may emit multiple cell records into one `NativeStream` lane after negative-shell ownership was added, but both `CpuTopologyOracle.Decode` and `CpuVertexAttributeOracle.Decode` still consume only the first record. Make both decode every record so minimum-face geometry and later normals/material parity comparisons are trustworthy.
