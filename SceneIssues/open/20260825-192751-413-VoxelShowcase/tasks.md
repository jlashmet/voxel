...
- [ ] **Remove obsolete minimum-face GPU fallback.** `ExactSnapshotClassificationJob.LowBoundaryTouchesNoSolidHalo` was added before the GPU mesher learned the negative ownership shell. Commit `9275602c3610079a2966cd022b1a3f2fb13d8b62` later changed GPU regular-cell count/write to `(CellsPerAxis + 1)^3`, maps cells to `-1..CellsPerAxis-1`, and applies the same selected-inside ownership rule. Remove the stale classifier fallback and replace its old regression with a GPU negative-shell ownership regression so supported Smooth/Rounded boundary chunks stay on GPU.
