...
- [ ] **Move GPU eligibility/classification off the CPU for GPU-candidate chunks.** `ExactSnapshotClassificationJob` currently scans the exact snapshot before GPU routing. Implement a raw-brick GPU classification kernel in the existing count dispatch: one thread per dense cache brick (10^3 at step 1, 18^3 at step 2), with a mixed-brick thread scanning its immutable 512-voxel mirror payload. Atomically return `hasOwnedSolid`, `requiresContinuous`, and `unsupported` bits alongside the bounded count readback. Do not classify only the sampled density lattice: it can miss unsupported authored/faceted voxels that still require CPU fallback.
