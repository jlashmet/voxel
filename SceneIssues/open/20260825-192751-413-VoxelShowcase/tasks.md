...
- [ ] **Fix the CPU topology oracle for negative-shell lanes.** `TransvoxelTopologyJob` may emit multiple cell records into one `NativeStream` lane after negative-shell ownership was added, but `CpuTopologyOracle.Decode` still consumes only the first record. Make the oracle decode every record so minimum-face CPU↔GPU geometry comparisons are trustworthy.
