...
- [ ] **Move GPU eligibility/classification off the CPU for GPU-candidate chunks.** `ExactSnapshotClassificationJob` currently scans the exact snapshot before GPU routing. Implement raw-brick GPU classification in the existing sample/count dispatch: the first `BrickCacheEdge^3` sample threads each classify one dense cache brick (10^3 at step 1, 18^3 at step 2); a mixed-brick thread scans its immutable 512-voxel mirror payload. Mark unsupported semantics atomically in the existing bounded counters and make regular/transition count return zero so the existing CPU fallback owns unsupported/empty cases. Do not classify only the sampled density lattice: it can miss authored/faceted voxels.
- [ ] **Decode `PackedStorage` surface semantics before GPU reconstruction/classification.** `GpuVoxelBrickMirror` uploads `NativeArray<ushort>` surface semantics byte-for-byte, but `VoxelBrickDensity.hlsl` currently treats the ushort as the 32-bit runtime `VoxelSurfaceSemantics.Packed` layout. Add the exact storage decode (style bits 0–4, coating 5–8, flags 9–10, detail 11–15) when reading mixed voxels, and add a real coating/detail parity regression. Existing simple Smooth/Rounded fixtures have zero upper bits and therefore do not prove this path.
