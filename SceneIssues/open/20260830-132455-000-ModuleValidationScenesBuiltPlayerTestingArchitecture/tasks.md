...
- [ ] Validate the thin canonical Water module scene in the exact-current-head built player. The scene attaches production `VoxelEngine.Showcase.WaterRenderingShowcase` plus the generic readiness probe under `Assets/VoxelEngine/Rendering/Validation/Water`; no bespoke Agent-8 tableau remains.
  ...
  - [BLOCKED] The same SpatialReservations manifest is present on current `master` and `fixes/agent-7`. Per assignment boundaries, agent-8 will not edit another assignment and will not weaken the repository-wide no-manifest acceptance. Do not submit a replacement targeted run until the external prerequisite is removed.
  - Pending independent work: inspect the canonical Water source scene/scenario and any retained prior canonical evidence that remains relevant; final acceptance still requires a fresh exact-current-head built-player run after blocker removal.
