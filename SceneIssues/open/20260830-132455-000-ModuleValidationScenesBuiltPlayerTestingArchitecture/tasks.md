...
- [ ] Replace the startup/prototype Water fixture with a representative production-quality module-local validation scene covering still water, shallow shoreline, river flow, waterfall/cascade, and terrain contact **without duplicating the canonical production Water showcase policy**.
  ...
  - After the second materially different scene-level correction path, ownership/reuse review isolated the more fundamental defect: Agent-8 is independently authoring Water showcase policy while agent-9 owns the production `WaterRenderingShowcase`. Agent-9's scene is a thin scene shell attaching `VoxelEngine.Showcase.WaterRenderingShowcase`, and its semantic regression `PortableShowcaseWorldAuthorsIndependentWaterProfilesThroughCanonicalStorage` already proves independent still/river/waterfall production semantics without relying on visual PlayMode evidence.
  - **Blocked external prerequisite:** current `master` remains `2edf4c2e151492f67c4a1c1b846a9b7948284aba`; agent-9's Water showcase is still only on `fixes/agent-9`. Per assignment boundaries, do not copy/cherry-pick or modify agent-9's work and do not make a third independent tableau tweak. When that canonical composition lands on `master`, adapt the module-local Water validation scene to attach/reuse it and update Water module test metadata to semantic production regressions as needed.
