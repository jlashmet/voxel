...
- [x] Reconcile `tools/tests/test_run_module_validation.py` with the final assembly-based runner contract. Source audit found the regression still supplied legacy `filter` metadata and expected obsolete error text; fixed at `925b4c6af0791597a91d451d0c917b6eb90d85f5`. Local execution is unavailable because this shell cannot resolve GitHub/Unity, so exact-SHA CI remains execution proof.
- [ ] Validate the thin canonical Water module scene in the exact-current-head built player. The scene attaches production `VoxelEngine.Showcase.WaterRenderingShowcase` plus the generic readiness probe under `Assets/VoxelEngine/Rendering/Validation/Water`; no bespoke Agent-8 tableau remains.
  ...
  - Run `33464201573` validated exact feature `7d88992a6a296c4dcb743c6ad219ec7ebfec77a3` and failed before Unity because the separate SpatialReservations assignment reintroduced `Assets/Game/WorldBuilder/Generation/Validation/SpatialReservations/spatial-reservations.module-validation.json`.
  - [BLOCKED] The same SpatialReservations manifest is present on current `master`, current `fixes/agent-8`, and the separately owned SpatialReservations work. Per assignment boundaries, agent-8 will not edit another assignment and will not weaken the repository-wide no-manifest acceptance. Do not submit a replacement targeted run until the external prerequisite is removed upstream and integrated.
  - Static audit confirms current `WaterDemo.unity` is a thin production consumer and `WaterDemo.player-scenario.json` contains only executable timing/capture/assertion behavior. Final acceptance still requires a fresh exact-current-head built-player run after blocker removal.
- [ ] Review final diff against all 18 acceptance criteria and complete SceneIssue metadata/closure only after canonical Water reuse, production-quality standalone Water evidence, and green exact-SHA CI exist for the final feature head.
  ...
  - Criteria 14 and the final proof portion of 5/6/7/13 require the blocked exact-head canonical Water -> Kentridge run and direct standalone-frame review.
