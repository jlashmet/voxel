...
- [x] Reconcile current master instead of rewriting stale tests/adjacent production. Feature was 216 commits behind at merge base `142b1134bd9d6a9eb1d60e55a296afaf6d9e7b3e`; master merged normally at `0b941fc62c87841bca949df32cf9ee3d6a4ded67` through PR #201; no force/synthetic merge.
- [x] Preserve master’s five newly added top-level EditMode regressions while maintaining module ownership: Kentridge road composition, terrain grading envelope, world-road presentation -> WorldBuilder; road surface detail -> Rendering; continuous terrain corridor rasterisation -> Structures. Delete temporary/top-level copies.
...
- [x] Before another behavioral fix, isolate a minimal repeated-symptom root cause. The renamed WorldBuilder suite still contained broad former repository-wide tests; moving them under production module roots caused old filesystem scanners to treat `/Tests/` source/asmdefs as production, while multiple guards still referenced pre-cutover WorldGen package paths or deleted Showcase runtime/gallery/traversal assets.
- [x] Correct only demonstrated migration defects: exclude `/Tests/` and `/Editor/` from production/constitution scans; retarget Kentridge/WorldGen ownership and shape-program guards to `Assets/Game/WorldBuilder/Generation`; validate current Showcase composition ownership instead of deleted scene runtime filenames; remove obsolete WorldbuildingGallery and Showcase traversal regressions whose production targets no longer exist. Do not change adjacent production behavior or the foreign SceneIssues those stale tests referenced.
