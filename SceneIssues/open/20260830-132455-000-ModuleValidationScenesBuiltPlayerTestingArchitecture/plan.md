...
- Exact-SHA run 33337814842 passed focused + automatic validation but direct Water review was a flat clear-colour frame. Minimal repro from `player-run.log`: `WATER_VALIDATION shallow shoreline probe mismatch at (208,112): expected 179/3, got 182/1`. The organic river bank was authored after the pool and overwrote that overlapping shoreline coarse column. The repair makes river/bank authoring precede the pool shelf so the shoreline owns the confluence column without weakening any probe.
- The same run also demonstrated a fail-closed gap: before module-owned required log assertions, an early-aborted composition could still produce enough blank screenshots for the generic runner to pass. Water now requires `WATER_VALIDATION ready:` and Kentridge requires `KENTRIDGE_WORLD_LAYOUT`, keeping semantic readiness policy in each scenario rather than adding feature knowledge to shared harness code.

## Blast radius / cost
CI/orchestration, validation assets, tests, and docs only; no authoritative gameplay runtime behavior changed. Measured automatic validation cost is 167.71 seconds for the earlier Water + Kentridge path. The current fix is confined to Water validation composition/content and module-owned validation metadata; shared renderer/harness thresholds remain unchanged.

## Current commit
Shoreline-overlap/fail-closed checkpoint: c9e4c318dd6206306b5d33c436f549acc49531b2
