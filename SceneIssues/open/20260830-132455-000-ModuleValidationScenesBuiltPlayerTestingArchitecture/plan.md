...
- Direct review of that run confirmed the original rendering-path defects are gone, but rejected the remaining presentation as a different composition defect: the wide camera exposed the artificial terrain apron and an oversized foreground pool dominated the frame.
- First scene-only presentation repair reduced pool footprints and tightened/lowered the overview. Exact-SHA run `33343070296` stayed green, selected the same Water + Kentridge flow, asserted both `WATER_VALIDATION ready:` and `KENTRIDGE_WORLD_LAYOUT`, and measured 168.20s total (12.55s focused test, 46.95s Water player, 108.70s Kentridge player). Direct frame review still rejected the overview because the required Water behaviors remained visually compressed into an atlas-like scene.
- Second materially different presentation approach replaces the single overview with a deterministic three-shot camera tour aligned to the existing capture cadence: still pool + shallow shoreline, descending river, then cascade + receiving-pool terrain contact. The production near-field Water renderer, authored storage, readiness assertion, and shared capture harness remain unchanged. If this same visual-acceptance symptom persists, isolate a minimal presentation repro/root cause before any further visual fix.

## Blast radius / cost
CI/orchestration, validation assets, tests, docs, and one composition-assembly reference only; no authoritative gameplay runtime behavior changed. The latest automatic Water + Kentridge flow measured 168.20 seconds. Current visual polish remains confined to Water validation composition and uses existing production rendering/composition APIs.

## Current commit
Water deterministic camera-tour checkpoint: `1ff024afc9d0bf9235b7c3dba0921b97e2f42c35`
