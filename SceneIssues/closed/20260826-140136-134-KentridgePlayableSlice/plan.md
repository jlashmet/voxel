...
- **Replay tooling masks the fix:** evidence-only; CI replay proves frozen pose first, then releases gameplay and disables the capture overlay.
...
- CI artifact was inspected directly. Its final 1928×900 real-player frame places the player inside the pub looking through the doorway; replay/F8 overlays are absent. `verification-replay.txt` records the runtime evidence.
- Required repository evidence is now committed as `verification-final.jpg`: JPEG quality 40, exactly 40% of the original 1928×836 capture, **771×334**, generated from that clean replay and visually inspected.
...
- Current master contains the production fix/regression and newer shared replay/capture tooling; the final feature-only delta is limited to this assigned SceneIssue evidence/bookkeeping.
- All gates complete; issue remains closed as fixed.
