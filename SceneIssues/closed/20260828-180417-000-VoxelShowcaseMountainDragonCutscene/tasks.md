...
- [ ] Shared renderer prerequisite: near-surface publication/convergence must stop producing torn/floating black strips/holes before Mountain Dragon visual acceptance; do not modify the shared renderer from agent-4. Renderer source `72634a3ca8e1dc1288469037ac930ee283aff129` / request `a16497220a861976e0a95a2cd9a1eee1d93baac7` / run `34024202854` failed required automatic module validation and has not landed on master. After a validated prerequisite lands, merge current master and revalidate exact built-player output.
- [x] Ensure an unmodified production capture with substantial error-magenta pixels fails the visual gate rather than silently passing; focused acceptance-capture gate and regression are present. This does not replace human visual review.
