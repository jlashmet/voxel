...
- [x] Merge compatibility-relevant master `b56436f198702fa91bfaccec69a30df25a52a2cd`, including the renderer publication-deadline repair, into `fixes/agent-4` via synchronization PR #334; resulting production source `450f275d3966fb69834ac41c7177aece3fea3355`.
- [x] Review the post-sync master-to-feature diff: 12 commits ahead, 0 behind at `b56436f...`, with that master as the merge base and the exclusive diff assignment-scoped with no renderer production edits.
- [x] Recheck current master `5c9a715a42735a0b573f436af1e3700dff2f591c`: the one new master commit is only Mounting Force town-art-direction closure/art material and does not change validation infrastructure or Mountain Dragon compatibility. Live branch state is 13 ahead / 1 behind with merge base `b56436f...`; defer this unrelated merge until compatibility requires it or the final-promotion sync.

## Revalidation and promotion
- [x] Revalidate the exact current production source's focused regression: run `34200864820` attempts 1 and 2 both pass all persistent/EditMode phases (including Rendering) and pass `ShowcaseStartupBakeArtifactTests.CurrentSourceBakeExportsPayloadAndMatchingManifest` 1/1 with zero failures.
