...
1. Fetch `origin`.
2. Merge the then-current `origin/master` into `fixes/agent-7` (do not rebase or cherry-pick another agent's branch).
...
6. Keep `ci-test/fixes/agent-7` as the only targeted-CI transport; never replace queued/running CI.
