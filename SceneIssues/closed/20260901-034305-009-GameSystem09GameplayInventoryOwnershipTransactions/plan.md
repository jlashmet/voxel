...
- SceneIssue closure bookkeeping committed at `ef2c1a641d3ce67ef1de5c140ee1d1098a30b6c2`: every task is checked, the directory moved directly from `open/` to `closed/`, and `status=fixed`/`resolvedUtc` plus resolution/regression/fix fields are populated.
- Current master `81ffa4bbc76c3feb6e0bde2376065b4144f3f10a` (Combat/Vitality-only delta relative to the previously integrated master) was merged into the closed feature head as true two-parent commit `f76d563cc392c70b2aecd18ef8c936d9d4099082`; comparison confirms that master is its merge base and the branch is not behind.

## Promotion status

- A required non-force update of `master` from `81ffa4bbc76c3feb6e0bde2376065b4144f3f10a` to exact feature head `f76d563cc392c70b2aecd18ef8c936d9d4099082` was attempted and rejected by GitHub with HTTP 422.
- Active repository ruleset `20911007` requires status check `affected` and requires changes through a pull request. It has no bypass actors and reports `current_user_can_bypass: never`.
- SceneIssue workflow forbids creating PRs/alternate transports for this worker and explicitly requires the exact feature head to be pushed non-force to master. A GitHub PR merge would also create/rewrite a different commit instead of making master the required exact feature SHA.
- Therefore all T09 acceptance, closure, and current-master synchronization are complete; exact-head master promotion is externally blocked by repository policy/authorization and cannot be completed without changing that policy or granting a bypass.
