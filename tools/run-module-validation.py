#!/usr/bin/env python3
"""Execute a module-validation plan: focused Unity tests, then built-player validations."""
...
def run_test(unity: str, item: dict, root: Path) -> float:
    ...
    test_filter = item["filter"]
    safe = "".join(c if c.isalnum() or c in "-_" else "_" for c in module + "-" + platform)
    out = root / "Tests" / safe
    ...
    args += ["-projectPath", str(Path.cwd()), "-runTests", "-testPlatform", platform,
             "-testFilter", test_filter, "-testResults", str(xml), "-logFile", str(log)]
    env = os.environ.copy()
    env.update({"UNITY_BIN": unity, "UNITY_MAX_RSS_MB": "14336", "UNITY_MAX_MINUTES": "4"})
    started = time.monotonic()
    subprocess.run(args, check=True, env=env)
    if not xml.is_file():
        raise SystemExit(f"ERROR: required focused test produced no results: {module} {test_filter}")
    cases = ET.parse(xml).getroot().findall(".//test-case")
    if not cases:
        raise SystemExit(f"ERROR: required focused test matched zero tests: {module} {test_filter}")
    failed = [c for c in cases if c.get("result") not in ("Passed", "Success")]
    if failed:
        raise SystemExit(f"ERROR: required focused test failed: {module} {test_filter}")
