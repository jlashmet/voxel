...
- Latest accepted code gate: `b4fbfb180831a588996d398442a66c38949771a3` — 382 tests, 369 passed, exactly the same 13 known baseline failures. Accepted Edits server capability slice: Edits.Api owns `IAlterationApplier`; server/session code consumes the API capability; `ServerDeterministicAlterationApplier` and the Net-owned applier interface are gone; Net still has no Edits.Runtime reference.
