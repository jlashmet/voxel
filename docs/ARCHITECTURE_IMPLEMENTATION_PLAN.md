...
| 13 — Composition/Core deletion | **In progress — current** | final inventory complete; WorldGen direction accepted; Terrain/Storage ownership extracted; Core deleted; functional Composition Storage bootstrap accepted; far-field, CharacterMotor, network presentation, GpuDebris and VoxelShowcase storage seams are API-only; ShowcaseCatalogue consumes Structures.Api only; mixed-brick capacity and lookdev renderer configuration are Composition-owned; CompactFps consumes Rendering.Api diagnostics; TerrainLookdev physical storage adapter construction is consolidated in its owner; empty Tools shells deleted; 387/374/13 baseline preserved | migrate remaining Showcase concrete Runtime wiring, remove residual Core guard literals/temporary exceptions, final dependency report |
...
- Latest accepted code gate: `a0528159ada889c59190888840523e6fb8c05a10` — 387 tests, 374 passed, exactly the same 13 known baseline failures. Integrated Cutover 13 run `31899261112` accepts Composition-owned mixed-brick capacity sizing and render-environment wiring, Rendering.Api surface timing diagnostics for `CompactFpsOverlay`, the final `VoxelShowcase` Storage.Api surface read, and consolidation of repeated TerrainLookdev physical storage adapter construction into its owner.
