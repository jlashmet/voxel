...
- Latest accepted code gate: `e0cdc61099ab3a1ae18af06a0b50ffa2f36af90c` — 382 tests, 369 passed, exactly the same 13 known baseline failures. This gate accepts the retained-profile Rendering cutover: `VoxelWorldView`, `VoxelSurfaceScheduler`, and `CpuTransvoxelChunkCache` consume Storage.Api `IProfileBlockReadSource`, the read contract exposes `Count`/`Version`/immutable snapshots, and Rendering no longer needs the concrete mutable `ProfileBlockStore` or its Structures-side writer interface. The preceding accepted gate `37241eb3c1c1b5afe50b3e6729bbf4ae8acfc935` accepts the physical move of `FeatureGeneration`, `ShapeProgram`, `PrimitiveRasteriser`, `ArchFeature`, `BondedBlockVeneer`, and all feature emitters into `Structures/Runtime`, with Unity GUIDs preserved; only Showcase/bootstrap, editor capture tooling, and implementation-focused tests reference `VoxelEngine.Structures.Runtime`. Engine subsystems and MountingForce WorldGen did not gain a Runtime reference. The earlier accepted gate `eee4fe26ae0c87a882389e210b38d171bb80aa2c` routes retained-profile authoring through `Structures.Api.IProfileBlockWriter` instead of the concrete mutable store and also returned 382/369 with the exact same 13 failures. The `023a8fce13d5203bb80e4b85fab178048e6caf2f` gate accepts the retained-profile logical value/read contract in Storage.Api and Storage.Api whole-cell block authoring. The baseline previously shrank from 15 to 13 because canonical `EmitBox` encoding fixed `KentridgeUpperSkybridgeTests.SkybridgeCatalogueCreatesOneOpenHardUpperStreetWithoutRoadPiers` and `KentridgeVerticalFrontageTests.VerticalFrontagesEmbedIntoTerraceAndEndOnAuthoredDownhillEdge`; no new failures were introduced.
