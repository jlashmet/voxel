...
- Latest accepted code gate: `dd7d5013c1409d4efe0dbfed3672cb9b5ab13ecd` — 382 tests, 369 passed, exactly the same 13 known baseline failures. This gate accepts the `VoxelBrush` Storage-boundary cutover: the brush now depends only on Storage.Api `IRegionReadSource`, `IRegionMutationStore`, and `IMaterialAuthoringCatalogue`; block-granular column writes still materialize/collapse once per logical block, whole authored blocks use `SetWholeCellBlock`, and the old `RegionTable`/`BrickPool`/`VoxelAccess` ownership plus `Table`/`Pool` escape hatch are gone. CastleBuilder, Showcase, tests, and editor capture callers construct Storage capability adapters instead of passing physical storage into the brush. The preceding accepted gate `e0cdc61099ab3a1ae18af06a0b50ffa2f36af90c` accepted the retained-profile Rendering cutover at the same 382/369/13 baseline.
