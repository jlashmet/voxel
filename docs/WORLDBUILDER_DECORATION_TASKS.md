...
- NUnit regressions are committed for scheduling, context variation, cave reuse, render backends, runtime batching/detail policy, persistence deltas, dining semantics/integration including a 64-seed great-hall pass, storage-family determinism/placement, lighting-family placement/effect policy, textile/martial display placement and thin-surface batching, a 128-seed bedroom stress pass, representative castle integration, and the existing castle build progression path. They still require execution in the Unity test environment/CI.
