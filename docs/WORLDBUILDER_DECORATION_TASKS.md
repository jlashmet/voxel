...
- Region-aware content is a first-class requirement. Kentridge, Hightown, Moordell, Rossdam, Fairy Village and Orc Village have voxel-owned decoration profiles informed by the imported Mounting Force world/visual guidance.
...
- The stable content manifest now contains **400 implemented archetypes**. The encoding leaves room through ID 1023 for continued expansion without renumbering the first 400.
- IDs 261-280 implement monster-lair/creature-occupation content; 281-300 adventurer-guild/quest/caravan; 301-320 magical-nature/fae; 321-340 traps/puzzles/dungeon mechanisms; 341-360 temple/shrine/sacred magic; 361-380 wizard-school/library; 381-400 cursed/haunted/aftermath content.
- Region-aware optional selection/presentation is implemented for newer content paths and explicit region-density logic exists for magical nature, sacred spaces and scholarly magic. Older 1-260 content still needs systematic migration onto the same policy.
...
- Unity/CI execution and visual/performance evidence remain separate completion gates. Source regressions are not counted as executed validation.
...
- [x] **DEC163** Reach **400 stable fantasy archetypes** with canonical stable IDs, render/backend paths, and representative scene/regression source.
...
- [ ] **DEC149** Expand dungeon/prison/secret-passage content beyond the current prison and trap/puzzle slices.
- [x] **DEC151** Expand chapel/temple/shrine/ritual content through IDs 341-360, with VillageShrine, GrandTemple and SacredCrypt source scenes.
- [x] **DEC152** Expand library/wizard-study/school/scholar content through IDs 361-380, with SpellClassroom, WizardLibrary and ForbiddenArchive source scenes.
...
- [x] **DEC159** Add the first ruin/abandonment/magical-aftermath pack through IDs 381-400, including cursed, haunted and corrupted scene variants.
- [ ] **DEC160** Add regional/faction/cultural fantasy dressing beyond the current six-region presentation profiles.
...
- [ ] **DEC190** Add broader treasure/loot-display vocabulary beyond current lockboxes, chests, reliquaries and trapped/cursed chest content.
- [x] **DEC191** Add trap/puzzle/environmental-interaction dressing at IDs 321-340, with TrapCorridor, PuzzleChamber and TreasureVault source scenes.
- [ ] **DEC192** Add fantasy food/feast variety beyond the current kitchen/bakery/brewery/winery catalog.
...
- [x] **DEC201** Add the initial region-aware content-weighting path without changing required scene anchors or stable IDs.
- [x] **DEC202** Add the initial region-aware presentation override layer so the same semantic prop can use settlement-specific materials/ornament without duplicate IDs.
...
- [ ] **DEC204** Expand region-density source coverage into older 1-260 scene families in addition to newer guild/nature/sacred/scholar comparisons.
...
## New 321-400 composition milestones

- [x] **DEC209** Magical-nature source compositions: EnchantedGrove, FairyClearing and DruidShrine.
- [x] **DEC210** Trap/puzzle source compositions: TrapCorridor, PuzzleChamber and TreasureVault.
- [x] **DEC211** Sacred source compositions: VillageShrine, GrandTemple and SacredCrypt, with Sacred-region density bias.
- [x] **DEC212** Wizard-school source compositions: SpellClassroom, WizardLibrary and ForbiddenArchive, with Scholar-region density bias.
- [x] **DEC213** Cursed/haunted source compositions: CursedLaboratory, HauntedChamber and CorruptedRuin, with condition-driven aftermath density.

## Larger composition milestones
...
- [ ] **DEC193** Wizard tower composition spanning study, alchemy, enchanting, observatory, familiar and school/library spaces.
