...
- [ ] Fix any visual/staging fidelity defects found in capture and repeat the smallest relevant validation until green.
  - Current implementation candidate is the bounded world-voxel cutaway at feature SHA `3b22621072ca755811ad9fc848810e433822ffb7`. Treat it as in-progress until the focused production presentation regression executes non-zero tests and passes, then a fresh standalone-player capture from the same implementation state is inspected at full resolution.
  ...
  - Next evidence gate: re-run `KentridgeOpeningPresentationTests.OpeningUsesOverheadRoofCutawayAndFadesAroundHandoff` once the shared Unity editor is clear; only if it is green, run and inspect a fresh standalone-player Kentridge capture before deciding whether the composition/cutaway fix is complete.
