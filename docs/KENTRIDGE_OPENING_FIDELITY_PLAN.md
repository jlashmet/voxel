...
- [ ] Fix any visual/staging fidelity defects found in capture and repeat the smallest relevant validation until green.
  ...
  - Validation boundary: entrance/carve/anchor alignment is green through Actions run `32563463855`; no visual-fidelity checkbox is earned by that structural proof. The next evidence must come from the renderer-bounds readability regression on this exact feature head, followed by a same-head standalone capture if green.
  - Next evidence gate: run the production renderer-bounds readability test first; only if it is green, run and inspect a fresh standalone-player capture from that same feature SHA before deciding whether the composition fix is complete.
