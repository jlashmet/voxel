...
It is subordinate to `AGENTS.md`, `docs/WORLDBUILDER_RUNTIME_INTEGRATION.md`, and `docs/MOUNTING_FORCE_WORLD_HANDOFF.md`. The legacy Mounting Force source defines story/cutscene truth; the voxel project may regenerate physical layout but must not invent or silently rewrite recovered narrative facts.
...
- [ ] Prove the rewritten cutaway through the focused production presentation regression on the exact implementation SHA.
  ...
  - Exact-implementation request: CI branch `ci-test/agent/kentridge-integration-cleanup` was force-reset to feature code SHA `26a3367c7c452e7fd3d0892d9d5998c9d4431c76`; request commit `16bd965522fabe829d1428b1cc86e3e6885480df`; Actions run `32585922773`, job `97062153717`. The runner again found the live developer Unity editor (`6000.5.6f1`) and both AssetImportWorkers on `/Users/jlashmet/code/voxel` for the complete 60-second safety window. `Run requested test` was skipped, so this run executed zero Unity tests. The presentation test also has no standalone-player capture profile, so no screenshot evidence was produced. This remains an editor-contention blocker, not a code failure, and the gate stays unchecked.
  - Current-head retry: feature head `9d4b7dc0f7518298f888b4b4fb3ac676fc2f5693` still carries the same production cutaway code from `26a3367c7c452e7fd3d0892d9d5998c9d4431c76`; request commit `6d0bad92a928b1e02f5b1c005958481f035ac814`; Actions run `32586869038`, job `97064473739`. The runner checkout was `/Users/jlashmet/tmp/_work/voxel/voxel`, but the global two-editor safety guard correctly refused to launch a second Unity while the developer editor plus two AssetImportWorkers remained active under `/Users/jlashmet/code/voxel` for the full 60-second window. `Run requested test` was skipped and the capture helper reported no configured profile for this focused test, so zero Unity tests and zero screenshots were produced. The gate remains unchecked.
