...
- The historical multiview code had an important failure amplifier: `_nearest_foreground_uv()` stopped at the first nearby row containing foreground rather than the nearest foreground pixel in 2D. On thin/foreshortened regions this collapsed unrelated loops onto a narrow strip.
...
- Run #67 (`32044802894`) passed after that correction and materially improved sampling statistics: left large snaps dropped from 36.3% to 28.8%, right from 20.1% to 9.1%, and global p95 snap distance from 100 px to 71 px. The corrected render remained visibly unacceptable, so orientation was a real defect but not the dominant remaining one.
- Commit `4f045f9f0098486932b6034e88f5cefa275d1e23` replaced the first-row fallback with a true 2D nearest-foreground search. Run #68 (`32045372505`) passed and dropped global p95 correction from 71 px to about 26 px; front/back maximum corrections fell to roughly 37/35 px. The old ladder-like striping was substantially reduced, proving the fallback bug was a major contributor, but the render still contained neutral wedges and collapsed side-arm/hair regions.
- Commit `2caa178fda385b0106e35da430f8edf2a7266f34` redirects high-risk outer-span side polygons to front/back projection so the T-pose arm-length X axis is not discarded. Run #69 (`32045765227`) passed with 1,538 outer-side redirects. Arm-length variation improved, but broad pale/gray wedges remained on the torso, limbs, and hair, so the arm redirect is necessary but not sufficient.
- Commit `a698347f4c50d45a01baf7468ea428ec194cc228` added full-resolution edge padding to test whether UV-triangle interpolation through neutral source canvas causes the remaining wedges. Run #70 (`32046136715`) completed the build and base verifier successfully; a later push cancelled it during artifact upload before visual proof could be recovered. Its log shows all four full-resolution padding passes together took only about 1.2 seconds, so full-resolution padding is cheap enough to retain while testing.
- Commit `a4626ba13c56b3ec8c188e22f334d8e1f71db860` experimentally moved padding after each source was reduced to its 512 px atlas tile. Run #71 (`32047153559`) passed and published proof, but the render still showed the same gray wedges. Inspection of its atlas showed entire rows copied from gray pixels because the resampled source image and nearest-row resampled mask were not perfectly aligned at silhouette endpoints. This optimization was disproven and reverted; the published #71 proof is retained as evidence.
- The remaining gray wedges are therefore not explained solely by the original large-snap bug, outer-arm axis collapse, or simple atlas-canvas interpolation. The next localization step must determine whether neutral pixels are entering through mask-edge samples, per-polygon UV interpolation, or visibility/occlusion errors before adding broader heuristics.
...
The generation/reconstruction and canonical-rig pipeline remain functional. Projection instrumentation, source orientation, normal-space handling, true nearest-foreground correction, and outer-arm side-view redirection are all in place. Those changes have removed much of the original ladder/ribbon failure class, but run #71 proves substantial neutral wedges remain. Full-resolution atlas edge padding has been restored for the next isolated proof because its tile-resolution optimization was disproven. The next decision is evidence-driven: if full-resolution padding removes the neutral wedges, keep it and proceed to visibility/face work; if it does not, add diagnostics that sample polygon interiors/atlas colors to distinguish mask-edge contamination from occlusion/view-selection errors before changing projection policy again.

Unity staging now uses a character-owned `Resources/Characters/Madeline` package with separate Models, Materials, Textures, and Animations folders. The Humanoid FBX material is remapped to `Madeline_Body`, the prefab overrides its renderer slot explicitly, and `MadelinePreview` exposes the shared Idle, Walk, Run, CrouchIdle, Wave, and Shrug clips; rendered and animated appearance remain validation gates.
