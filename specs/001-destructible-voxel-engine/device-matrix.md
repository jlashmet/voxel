...
| Extracted surface geometry (GPU) | 1.25 GB | 640 MB | 320 MB |
...
budget carries headroom above the frustum's own working set rather than sitting exactly on it.
Sized flush to the working set the arena thrashes instead of converging: eviction gives up one chunk
to admit the next and a handful of visible chunks never close. The
