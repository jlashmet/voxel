...
- [ ] Confirm the new per-fixture PlayMode shard layout no longer hits the Unity RSS watchdog on the current head. PR run 32014802229 proves crash/session isolation works and later shards continue, but the already-single-method `MemoryStaysWithinTierBudgetOverTwoHours` and `ContinuousTraversalOverKilometresShowsNoGaps` shards still hit 14,372 MB and 14,370 MB respectively against the unchanged 14,336 MB ceiling in PR run 32019741845; further YAML splitting cannot solve those two tests.
