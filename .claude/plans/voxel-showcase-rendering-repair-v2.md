...
- [ ] Determine which fallback-admission predicate is false for those eight current-empty frustum chunks (`exact-owned` classification versus profile-backed exclusion), add a focused regression for that predicate, and fix only the first proven false predicate before changing coarse geometry again.
...
Pinned lifecycle run 32032548787 on source `4842bd44` resolves the next coarse-loss stage without another rendering change: step-4 routing reaches 23 in-band chunks and 8 frustum chunks, but all 8 are already current-empty (`ready=0/empty=8`) and the feature-preserving fallback never schedules (`0/0/0/0`). The next task is therefore to isolate which admission predicate blocks fallback for those current-empty castle chunks—exact-owned classification or profile-backed exclusion—and fix that predicate before changing coarse geometry or crediting the newer atomic LOD active-coverage candidate.
