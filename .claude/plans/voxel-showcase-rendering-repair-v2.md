...
- [x] Add step-4 fallback lifecycle diagnostics that separately count fallback scheduling, worker completion, non-empty HLOD output and successful GPU publication; expose the counters through `VoxelSurfaceMetrics` and the existing LOD failure diagnostic without changing renderer behavior.
...
PR #88 run 32028393584 (`63577998`) validates the step-4 false-empty fallback policy/regression and the baked startup step, but production PlayMode remains red. Coarse lookdev still reports 48 silhouette holes, `LodRenderingTests` still reaches a ready/idle step-4 state with seven resident chunks but zero visible draws, and the 10-second showcase still has 1,539 missing visible chunks. This disproves the focused fallback as the complete production fix: the next gate is to identify whether those requested step-4 chunks enter fallback and finish empty, never satisfy exact-owned-solid, or are excluded by authored-profile handling. Do not broaden step-4 HLOD or change LOD thresholds until that cache-lifecycle cause is measured.
