...
PR #88 run 32028393584 (`63577998`) validates the step-4 false-empty fallback policy/regression and the baked startup step, but production PlayMode remains red. Coarse lookdev still reports 48 silhouette holes, `LodRenderingTests` still reaches a ready/idle step-4 state with seven resident chunks but zero visible draws, and the 10-second showcase still has 1,539 missing visible chunks. The next required evidence is the lifecycle diagnostic run: whether requested step-4 castle chunks enter fallback, complete with geometry and publish, or are adjudicated empty before that path. Do not broaden step-4 HLOD, change LOD thresholds, or pursue the newer unused priority enum until this cache-lifecycle cause is measured.
