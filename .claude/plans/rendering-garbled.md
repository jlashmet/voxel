...
- [ ] Reimplement the one-call solid submission without `SV_DrawID`.
...
- [ ] Test the next smallest addressing hypothesis before changing any geometry producer: remove `UnityIndirect.cginc`/`InitIndirectDrawArgs(0)` from the indexed vertex path and consume hardware `SV_VertexID` directly. The shader does not need command ID, instance ID, or argument-buffer values; this isolates whether command-zero vertex-ID fixup is corrupting later commands while preserving the same chunk-local indices, `startIndex`, `baseVertexIndex`, arena, visibility, LOD and budgets.
- [ ] Run the direct-`SV_VertexID` source regression red then green on the dedicated CI branch, followed by the same five-frame standalone Metal comparison. Reject it immediately if the survey corruption remains.
- [ ] If a Metal-correct one-call path is found, preserve or restore a bucketed non-game/editor fallback without changing the proven game-camera path, then rerun the focused contract and player comparison on that final integration.
- [ ] Compare the final optimized stable 35-60 s FPS/frame-time window against the clean bucketed baseline. Keep the optimization only if it produces a repeatable frame-time improvement without reducing rendered coverage or fidelity; otherwise retain the bucketed renderer and identify the next measured submission hotspot.
