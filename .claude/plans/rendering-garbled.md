...
- [ ] Reimplement the one-call solid submission without `SV_DrawID`. Candidate 1 reconstructs the exact measured fast path and changes only the shader lookup from command-relative `GetIndirectVertexID` to base/absolute `GetIndirectVertexID_Base`; do not accept it until the real Metal player proves the change removes the original cross-world corruption.
...
- [ ] If candidate 1 is visually correct, preserve or restore a bucketed non-game/editor fallback without changing the proven game-camera path, then rerun the focused contract and player comparison on that final integration.
