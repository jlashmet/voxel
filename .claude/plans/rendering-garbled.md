...
### F. Research the indexed-indirect API contract before any revisit

This section records documentation/source research only. It does not reintroduce the rejected renderer and does not supersede the measured visibility/culling hotspot above.
...
- [ ] Before another production attempt, verify the exact `UnityIndirect.cginc` shipped with Unity `6000.5.6f1` against the public helper mirror, then build a microscopic current-doc Metal reproducer with two or three visibly distinct triangles. Isolate `startIndex`, `baseVertexIndex`, and command selection independently: (1) nonzero base vertex with the simplest possible index range, (2) nonzero start index with base vertex zero, (3) both nonzero, and (4) `commandCount = 1, startCommand = 1` versus `commandCount = 2`. Keep the shader on Unity's current documented `InitIndirectDrawArgs(0)` + `GetIndirectVertexID` path and encode `GetCommandID(0)` into color/position. Do not change voxel production code until this test proves which Unity 6000.5.6f1 Metal contract is actually honored.
