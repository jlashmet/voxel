...
- Targeted CI is currently blocked at runner availability rather than by the Arch request itself. The repository has exactly two active `ci-test/*` mailboxes: `ci-test/agent/arch-reference-match` and `ci-test/fixes`. The SceneIssues request head `6f372a680ce19e48386831a0d209e1cde13654a4` also has no `ci/single-test` status, so neither targeted job has reached its first self-hosted step. Current master SceneIssues captures were authored from `OSXEditor` between 2026-08-23 01:38 and 01:46 UTC, confirming Unity Editor was actively in use on the Mac during the earlier contention window. Do not interpret the missing Arch status as a test failure or advance the capture checklist until the self-hosted job actually starts.
