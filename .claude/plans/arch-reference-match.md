...
- The retained profile is definitively front-local and cannot explain a defect at the actual rear of the arch. `ArchBayFeatureDefinition` offsets the structural arch to `archOrigin.z = bayOrigin.z + 1`; then `ArchFeatureDefinition` authors the default retained layer at `FrontQ4 = archOrigin.z * 16 - 8` and `BackQ4 = archOrigin.z * 16 + 8`. With the ArchLookdev bay origin at Z=0 this is z=0.5 through z=1.5, while the default `Depth=12` structural `ArcWedge` spans z=1 through z=12. The retained layer's emitted back/shoulder/radial faces therefore remain at the structural front and cannot create or repair missing geometry at the true rear. Front disconnect and rear/soffit loss should be treated as separate paths unless the current screenshot proves the user's “back” view is only the front-local retained layer viewed from behind.
