...
- Branch repair needed to validate the existing fix: duplicate verification-only oracle code was
  committed twice and currently prevents `fixes` from compiling.
...
- Under the current AGENTS.md, final validation uses only `ci-test/fixes`; local Unity runs are
  diagnostic evidence and cannot establish completion.
...
- Removing only the duplicate copies restored local compilation. The focused coarse-density class
  then passed 5/5, and the related GPU/ownership/cutover/vertex selection passed 12/12 after updating
  one stale source-text assertion to the shader's current nested guard. These local runs guide the
  repair but must be re-proven through targeted CI.
...
- [x] Diagnose the duplicate-oracle compile failure and prepare the minimal deduplication.
...
- [ ] If the replay is clean, commit the feature/test result, resolve `issue.json` separately, push,
      and delete `ci-test/fixes`; otherwise update this plan before further production changes.
