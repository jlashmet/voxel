...
- [ ] Restore the full traversal PlayMode precondition to a **fallback-safe visibility gate**: require visible near geometry and require the far-field hole to stay closed whenever near coverage is incomplete, then begin movement. Do not wait for `MissingVisibleSolidChunks==0` before exercising traversal.
- [ ] Revert the invalid 1800-frame contract and exact full-traversal bake exclusion/source-contract assertions introduced by `8fe5c939`, `412c917c`, and `b5661592`.
- [ ] Establish a current baked-world input for final standalone acceptance without violating single-test latency. Preferred path: capture the fresh deterministic `ShowcaseWorld.bytes` from a dedicated under-five-minute bake request, commit that refreshed binary to the feature branch, then keep the final traversal/player request bake-free because the checked-in asset is now current.
