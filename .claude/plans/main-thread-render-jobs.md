...
- [x] Diagnose the first full `VoxelShowcase` acceptance attempt (`32575925066`) rather than changing renderer code speculatively. Its eight-worker PlayMode traversal expired before movement at the 1200-frame convergence cutoff with `known=6817 resident=696 dirty=1429 visible=696 missing=40 jobs=2`; the older one-worker run had been much farther away (`missing=534 jobs=12`). The accompanying 150-second standalone player did reach movement and settle.
- [x] Validate near/far fallback independently in the completed standalone run from `32575925066`: all 51 sampled diagnostic groups with `missingVisible>0` kept `HoleRadiusMetres=0` / far fallback under the near field, with zero sampled violations. Settled tail reached `missingVisible=0`, `leaseFail=0`, `reappeared=0`. Walking screenshots show continuous ground coverage; the first walking frame has a visible representation/LOD seam but no exposed world/sky hole.
- [x] Correct the full traversal validation harness without changing renderer behavior: exact traversal now uses a bounded 1800-frame initial convergence allowance (`8fe5c939a0b9694d82d3397d756fcf33a299b59f`), matching the existing full-showcase steady-state allowance; the exact renderer traversal skips the redundant ~198-second pre-bake while other full-showcase tests retain it (`412c917c30465c624d870a1f11e97a252169880b`); source contracts preserve the eight-worker route, five-minute CI policy, bake exclusion, and 1800-frame gate (`b5661592fece128edd14f5875ea69065b152f88e`).
- [ ] Re-run the corrected full `VoxelShowcase` traversal + 150-second real-player profile and require: PlayMode reaches movement and passes the near/far assertions, standalone player remains hole-free/fallback-correct, and the whole single-test workflow completes under five minutes.
