...
- [ ] Publish that generated binary once onto the existing CI branch so GitHub owns the blob, then attach that exact blob to the feature branch and remove the temporary publication/bake-artifact bridge. This is a transport step only; do not regenerate or reinterpret the bytes.
- [ ] After the refreshed checked-in asset is current, restore a bake-free exact full-traversal request so the final PlayMode + visible standalone acceptance fits the five-minute single-test policy.
