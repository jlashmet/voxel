...
- [x] Add a focused EditMode source contract for the fallback-safe traversal gate (`62b6d853`); CI request `a241cedd` is pending.
...
- [ ] Establish a current baked-world input for final standalone acceptance without violating single-test latency: run the one-shot bake request, capture the fresh deterministic `ShowcaseWorld.bytes`, commit that refreshed binary to the feature branch, then remove the bridge. Only after the checked-in asset is current may the final traversal/player request safely skip the ~198-second rebake.
