...
{
    /// <summary>
    /// Architectural form for one anonymous piece of block frontage. Unlike gameplay buildings this
    /// has no stable role identity; its identity is the deterministic block/run/site tuple.
    /// </summary>
    ...
    /// <summary>
    /// Grammar for anonymous urban fabric. The block plan supplies density, storey limits, frontage,
    /// court gaps, district and elevation. This layer supplies local architectural rhythm without
    /// changing any of those city-scale decisions.
    /// </summary>
    public static class KentridgeUrbanFabricGrammar
    {
            ...
            int siteIndex)
        {
            ...
            int width = 56 + (int)((h >> 3) % 7u);   // 56..62 dm
            int depth = 50 + (int)((h >> 7) % 9u);  // 50..58 dm
