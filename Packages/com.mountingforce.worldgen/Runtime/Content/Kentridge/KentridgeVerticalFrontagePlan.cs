...
namespace MountingForce.WorldGen.Content.Kentridge
{
    ...
    /// <summary>
    /// Settlement-scale contract for one occupied lower facade beneath an urban block. The zone is
    /// derived from the block's protected court-access edge, so vertical architecture follows the
    /// same frontage/court topology as the buildings above instead of becoming independent dressing.
    /// </summary>
    public readonly struct KentridgeVerticalFrontageZone
    {
        ...
        public KentridgeVerticalFrontageZone(
            ...
            int depthDm,
            int bayPitchDm,
            KentridgeVerticalFrontageStyle style)
        {
            Id = id;
            Band = band;
            District = district;
    ...
    public sealed class KentridgeVerticalFrontagePlan
    {
        ...
        public KentridgeVerticalFrontagePlan(List<KentridgeVerticalFrontageZone> zones)
        {
            _zones = zones;
        }
    }
    ...
    public static class KentridgeVerticalFrontagePlanner
    {
        ...
        public static KentridgeVerticalFrontagePlan Build(uint seed)
        {
            ...
            for (int i = 0; i < massing.Blocks.Count; i++)
            {
                ...
                if (block.Band == KentridgeUrbanBand.LowerWard)
                    continue;

                string runId = block.Id + "-"
                    + block.CourtAccessEdge.ToString().ToLowerInvariant();
                KentridgeFrontageRun run = FindRun(massing, runId);
                if (!run.IsHorizontal || !run.HasGap)
                    throw new InvalidOperationException(
                        "Kentridge vertical frontage requires a horizontal court-access run: "
                        + block.Id);
        ...
        private static KentridgeFrontageRun FindRun(
            KentridgeUrbanMassingPlan massing,
            string id)
        {
            for (int i = 0; i < massing.FrontageRuns.Count; i++)
                if (massing.FrontageRuns[i].Id == id)
                    return massing.FrontageRuns[i];

            throw new InvalidOperationException(
                "Missing Kentridge court-access frontage run: " + id);
        }
        ...
        private static void Validate(List<KentridgeVerticalFrontageZone> zones)
        {
            if (zones.Count != 6)
                throw new InvalidOperationException(
                    "Kentridge vertical frontage count changed unexpectedly.");
            ...
            for (int i = 0; i < zones.Count; i++)
            {
                ...
                if (zone.HeightDm <= 0 || zone.DepthDm <= 0 || zone.BayPitchDm <= 0)
                    throw new InvalidOperationException(
