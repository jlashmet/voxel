...
namespace Game.Structures.Runtime
{
    ...
    {
        public static uint SceneId(DecorationExpansion320SceneKind kind) =>
            0xE3200000u | ((uint)kind + 1u);
            ...
            switch (kind)
            {
                ...
                case DecorationExpansion320SceneKind.FairyClearing:
                    return new[]
                    {
                        ...
                        S(2, DecorationExpansion320Kind.FairyHouseNook, true, 6),
        ...
        public static int OptionalBudget(
            DecorationExpansion320SceneKind kind,
            ...
            in DecorationContext context)
        {
            ...
            if (profile.IsWellFormed && profile.Prefers(
                    DecorationRegionContentTags.Enchanted | DecorationRegionContentTags.Organic))
                budget += 2;
        ...
        private static DecorationExpansion320SceneSlot S(
            uint id,
            DecorationExpansion320Kind kind,
            bool required,
            ushort weight,
            DecorationSocketKind socket = DecorationSocketKind.Floor) =>
            new DecorationExpansion320SceneSlot
            {
                SlotId = id,
                Kind = kind,
                Socket = socket,
    ...
    public static class DecorationExpansion320SceneResolver
    {
            ...
            out DecorationPlacement[] placements)
        {
            ...
            for (int i = 0; i < slots.Length; i++)
            {
                ...
                if (!slots[i].Required)
                {
                    ...
                    if (profile.IsWellFormed && profile.Prefers(
                            DecorationRegionContentTags.Enchanted | DecorationRegionContentTags.Organic))
                        core[i].Weight = (ushort)(core[i].Weight + 5);
                }
            }

            if (!DecorationSceneScheduler.TrySelectAndOrder(
                    in context, sceneId, core,
                    DecorationExpansion320SceneCatalog.OptionalBudget(kind, region, in context),
            ...
            for (int i = 0; i < ordered.Length; i++)
            {
                ...
                DecorationPropDescriptor descriptor = DecorationExpansion320Catalog.Describe(
        ...
        private static DecorationExpansion320SceneSlot Find(
            DecorationExpansion320SceneSlot[] slots,
            uint slotId)
        {
            for (int i = 0; i < slots.Length; i++)
