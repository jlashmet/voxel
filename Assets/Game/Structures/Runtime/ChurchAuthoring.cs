...
namespace Game.Structures.Runtime
{
    ...
    public static class ChurchAuthoring
    {
            ...
            in ChurchConfig config)
        {
            ...
            authoring.HollowBox(naveMin,
                new int3(naveWidth, config.NaveWalls.Height, naveDepth),
                config.WallThickness, wallMaterial, false, false);
            ...
            if (config.AislesEnabled)
            {
                ...
                authoring.HollowBox(westAisleMin,
                    new int3(westAisleWidth, config.AisleHeight, westAisleDepth),
                    config.WallThickness, wallMaterial, false, false);
                authoring.HollowBox(eastAisleMin,
            ...
            authoring.HollowBox(sanctuaryMin,
                new int3(sanctuaryWidth, config.SanctuaryHeight, sanctuaryDepth),
                config.WallThickness, wallMaterial, false, false);

            AuthorInternalConnections(components, authoring,
                naveMin, naveWidth, naveDepth,
                westAisleMin, westAisleWidth, westAisleDepth,
                eastAisleMin, eastAisleWidth, eastAisleDepth,
                sanctuaryMin, sanctuaryWidth, sanctuaryDepth, in config);

            AuthorApse(authoring, origin, frontZ, in config);
            AuthorFacadeOpenings(components, authoring,
                naveMin, naveWidth, naveDepth,
                westAisleMin, westAisleWidth, westAisleDepth,
                eastAisleMin, eastAisleWidth, eastAisleDepth, in config);
            AuthorBellTower(components, authoring, origin, frontZ,
                naveMin, naveWidth, naveDepth, in config);

            AuthorRoof(components, authoring, naveMin, naveWidth, naveDepth,
                baseY + config.NaveWalls.Height, in config.NaveRoof,
                config.EntryFacing, in config.Palette);
            if (config.AislesEnabled)
            {
                AuthorRoof(components, authoring, westAisleMin, westAisleWidth, westAisleDepth,
                    baseY + config.AisleHeight, in config.AisleRoof,
                    config.EntryFacing, in config.Palette);
                AuthorRoof(components, authoring, eastAisleMin, eastAisleWidth, eastAisleDepth,
                    baseY + config.AisleHeight, in config.AisleRoof,
                    config.EntryFacing, in config.Palette);
            }
            AuthorRoof(components, authoring, sanctuaryMin, sanctuaryWidth, sanctuaryDepth,
                baseY + config.SanctuaryHeight, in config.SanctuaryRoof,
                config.EntryFacing, in config.Palette);
        }

        private static void AuthorFoundation(
            IStructureAuthoringSession authoring, int3 origin, in ChurchConfig config)
        {
        ...
        private static void AuthorInternalConnections(
            IStructureComponentAuthoring components,
            IStructureAuthoringSession authoring,
            int3 naveMin, int naveWidth, int naveDepth,
            int3 westAisleMin, int westAisleWidth, int westAisleDepth,
            int3 eastAisleMin, int eastAisleWidth, int eastAisleDepth,
            int3 sanctuaryMin, int sanctuaryWidth, int sanctuaryDepth,
            in ChurchConfig config)
        {
            ...
            Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
                config.WallThickness, in config.SanctuaryArch, 1, naveNorth, 0, 0, in config.Palette);
            Open(components, authoring, sanctuaryMin, sanctuaryWidth, config.SanctuaryHeight, sanctuaryDepth,
                config.WallThickness, in config.SanctuaryArch, 1, sanctuarySouth, 0, 0, in config.Palette);
            ...
            Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
                config.WallThickness, in config.AisleArch, count, west, 0, config.AisleArch.Spacing, in config.Palette);
            Open(components, authoring, westAisleMin, westAisleWidth, config.AisleHeight, westAisleDepth,
                config.WallThickness, in config.AisleArch, count, east, 0, config.AisleArch.Spacing, in config.Palette);
            Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
                config.WallThickness, in config.AisleArch, count, east, 0, config.AisleArch.Spacing, in config.Palette);
            Open(components, authoring, eastAisleMin, eastAisleWidth, config.AisleHeight, eastAisleDepth,
                config.WallThickness, in config.AisleArch, count, west, 0, config.AisleArch.Spacing, in config.Palette);
        }
            ...
            in ChurchConfig config)
        {
            ...
            if (config.BellTowerPlacement != ChurchBellTowerPlacement.FrontCentre)
                Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
                    config.WallThickness, in config.MainPortal, 1, south, 0, 0, in config.Palette);

            int windowCount = config.Window.MaxCountForSpan(config.NaveLength);
            if (config.AislesEnabled)
            {
                Open(components, authoring, westAisleMin, westAisleWidth, config.AisleHeight, westAisleDepth,
                    config.WallThickness, in config.Window, windowCount, west, 0, config.Window.Spacing, in config.Palette);
                Open(components, authoring, eastAisleMin, eastAisleWidth, config.AisleHeight, eastAisleDepth,
                    config.WallThickness, in config.Window, windowCount, east, 0, config.Window.Spacing, in config.Palette);
            }
            else
            {
                Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
            ...
            if (config.SideDoorsEnabled)
            {
                ...
                Open(components, authoring, westShell, westWidth, westHeight, westDepth,
                    config.WallThickness, in config.SideDoor, 1, west, 0, 0, in config.Palette);
                Open(components, authoring, eastShell, eastWidth, eastHeight, eastDepth,
                    config.WallThickness, in config.SideDoor, 1, east, 0, 0, in config.Palette);
            }
            ...
            if (config.ClerestoryEnabled)
            {
                ...
                Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
                    config.WallThickness, in config.ClerestoryWindow, count, west, 0, config.ClerestoryWindow.Spacing, in config.Palette);
                Open(components, authoring, naveMin, naveWidth, config.NaveWalls.Height, naveDepth,
                    config.WallThickness, in config.ClerestoryWindow, count, east, 0, config.ClerestoryWindow.Spacing, in config.Palette);
            }
        }

        private static void AuthorApse(
            IStructureAuthoringSession authoring, int3 origin, int frontZ, in ChurchConfig config)
        {
            if (!config.ApseEnabled) return;
            int localCentreZ = frontZ + config.NaveLength + config.SanctuaryLength;
            int2 rotatedCentre = StructureCardinalTransform.Point(new int2(0, localCentreZ), config.EntryFacing);
            ...
            authoring.Cylinder(cx, origin.y, cz, config.ApseRadius, config.ApseHeight,
                wall, config.ApseRadius - config.WallThickness);
            ...
            authoring.Box(carveMin, new int3(carveWidth, config.ApseHeight, carveDepth),
                config.Palette.Resolve(StructureMaterialRole.Opening));
            authoring.Cone(cx, origin.y + config.ApseHeight, cz,
                config.ApseRadius + config.SanctuaryRoof.EaveOverhang,
                config.ApseRoofHeight, config.Palette.Resolve(config.SanctuaryRoof.MaterialRole));
        }

        private static void AuthorBellTower(
            IStructureComponentAuthoring components,
            IStructureAuthoringSession authoring,
            int3 origin, int frontZ,
            int3 naveMin, int naveWidth, int naveDepth,
            in ChurchConfig config)
        {
            ...
            {
                case ChurchBellTowerPlacement.FrontCentre: towerX = -config.BellTower.Width / 2; break;
                case ChurchBellTowerPlacement.FrontLeft: towerX = -config.NaveWidth / 2; break;
                case ChurchBellTowerPlacement.FrontRight: towerX = config.NaveWidth / 2 - config.BellTower.Width; break;
                default: throw new System.ArgumentOutOfRangeException(nameof(config.BellTowerPlacement));
            }

            var local = new StructureFootprintRect(
                new int2(towerX, frontZ), new int2(config.BellTower.Width, config.BellTower.Depth));
            ResolveRect(in local, origin, config.EntryFacing,
                out int3 towerMin, out int towerWidth, out int towerDepth);
            authoring.HollowBox(towerMin,
                new int3(towerWidth, config.BellTower.Height, towerDepth),
                config.WallThickness, config.Palette.Resolve(config.BellTower.WallMaterialRole), false, false);

            Facing north = StructureCardinalTransform.FacingDirection(Facing.North, config.EntryFacing);
            Facing south = StructureCardinalTransform.FacingDirection(Facing.South, config.EntryFacing);
            Open(components, authoring, towerMin, towerWidth, config.BellTower.Height, towerDepth,
                config.WallThickness, in config.MainPortal, 1, north, 0, 0, in config.Palette);
            if (config.BellTowerPlacement == ChurchBellTowerPlacement.FrontCentre)
                Open(components, authoring, towerMin, towerWidth, config.BellTower.Height, towerDepth,
                    config.WallThickness, in config.MainPortal, 1, south, 0, 0, in config.Palette);
            ...
            if (config.BellTower.OpeningsEnabled)
            {
                ...
                Facing east = StructureCardinalTransform.FacingDirection(Facing.East, config.EntryFacing);
                Open(components, authoring, towerMin, towerWidth, config.BellTower.Height, towerDepth,
                    config.WallThickness, in config.BellTower.Opening, 1, south, 0, 0, in config.Palette);
                Open(components, authoring, towerMin, towerWidth, config.BellTower.Height, towerDepth,
                    config.WallThickness, in config.BellTower.Opening, 1, north, 0, 0, in config.Palette);
                Open(components, authoring, towerMin, towerWidth, config.BellTower.Height, towerDepth,
                    config.WallThickness, in config.BellTower.Opening, 1, west, 0, 0, in config.Palette);
                Open(components, authoring, towerMin, towerWidth, config.BellTower.Height, towerDepth,
                    config.WallThickness, in config.BellTower.Opening, 1, east, 0, 0, in config.Palette);
            }

            AuthorRoof(components, authoring, towerMin, towerWidth, towerDepth,
            ...
            if (config.SpireEnabled)
            {
                ...
                authoring.Cone(origin.x + centre.x,
        ...
        private static void Open(
            IStructureComponentAuthoring components,
            IStructureAuthoringSession authoring,
            int3 shellMin, int width, int height, int depth, int wallThickness,
            in OpeningConfig opening, int count, Facing facade, int groupOffset, int spacing,
            in StructureMaterialPalette palette)
        {
            var request = new StructureOpeningAuthoringRequest
            {
                ShellMin = shellMin, Width = width, Height = height, Depth = depth,
                WallThickness = wallThickness, Opening = opening, Count = count,
                Facade = facade, GroupOffset = groupOffset, Spacing = spacing, Palette = palette,
            };
            components.AuthorOpenings(authoring, in request);
        }

        private static void AuthorRoof(
            IStructureComponentAuthoring components,
            IStructureAuthoringSession authoring,
            int3 shellMin, int width, int depth, int roofY,
            in RoofConfig localRoof, Facing entryFacing, in StructureMaterialPalette palette)
        {
            ...
            {
                FootprintMin = shellMin, Width = width, Depth = depth, BaseY = roofY,
                Roof = roof, Material = palette.Resolve(roof.MaterialRole),
            };
            components.AuthorRoof(authoring, in request);
        }

        private static void ResolveRect(
            in StructureFootprintRect local, int3 origin, Facing facing,
