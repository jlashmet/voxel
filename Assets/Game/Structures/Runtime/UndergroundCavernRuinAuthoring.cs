...
namespace Game.Structures.Runtime
{
    ...
    public static class UndergroundCavernRuinAuthoring
    {
            ...
            in UndergroundCavernTraversalProfile traversalProfile)
        {
            ...
            // The deterministic cavern shell overlaps the final primary-route spans. Reuse the same
            // rounded protected-passage contract that is reasserted after optional visual finish so
            // both safety passes preserve equivalent floor/height/width overlap without carving long
            // planar side faces through the natural cavern.
            int protectedHalfWidth = math.max(6, caveConfig.TunnelWidth / 4);
            UndergroundCavernCirculationProtection.Reassert(
                authoring,
                in cavernBounds,
                in ruinBounds,
                destination.ExitFacing,
                width: protectedHalfWidth * 2,
                clearanceHeight: math.max(24, caveConfig.TunnelHeight - 4));
