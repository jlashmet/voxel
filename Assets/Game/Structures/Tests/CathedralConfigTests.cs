...
namespace Game.Structures.Tests
{
    public sealed class CathedralConfigTests
    {
        ...
        public void UndergroundAnchorsRotateWithCathedral(Facing entryFacing, Facing expectedCaveFacing)
        {
            StructureMaterialPalette palette = CastleStructurePalette.Compatibility;
            CathedralConfig gothic = CathedralPresets.Gothic(in palette);
            ...
            int2 expectedCryptXZ = StructureCardinalTransform.Point(
                new int2(gothic.CryptAnchor.LocalPosition.x, gothic.CryptAnchor.LocalPosition.z), entryFacing);
            int2 expectedCaveXZ = StructureCardinalTransform.Point(
                new int2(gothic.CaveAnchor.LocalPosition.x, gothic.CaveAnchor.LocalPosition.z), entryFacing);

            Assert.Multiple(() =>
            {
                Assert.IsTrue(gothic.IsWellFormed);
                Assert.AreEqual(
                    new int3(origin.x + expectedCryptXZ.x, origin.y + gothic.CryptAnchor.LocalPosition.y,
                        origin.z + expectedCryptXZ.y),
                    gothic.ResolveCryptAnchor(origin));
                Assert.AreEqual(
                    new int3(origin.x + expectedCaveXZ.x, origin.y + gothic.CaveAnchor.LocalPosition.y,
        ...
        public void GothicCryptCarveUsesRotatedLocalFootprint()
        {
            ...
            CathedralAuthoring.Author(components, session, origin, in gothic);

            int2 centreLocal = new int2(gothic.CryptAnchor.LocalPosition.x, gothic.CryptAnchor.LocalPosition.z);
            ...
            StructureFootprintRect rotated = StructureCardinalTransform.Rect(in cryptLocal, gothic.Church.EntryFacing);
            int expectedBottomY = origin.y - gothic.CryptTopOffset - gothic.CryptHeight;

            Assert.IsTrue(session.ContainsEmptyBox(
                new int3(origin.x + rotated.Min.x, expectedBottomY, origin.z + rotated.Min.y),
        ...
        public void GothicMainEntranceReachesApseAndSideChapel(Facing facing)
        {
            ...
            int firstChapelCentreZ = sanctuaryCentreZ - chapelGroupLength / 2 + gothic.SideChapelWidth / 2;
            int2 chapelLocal = new int2(-church.SanctuaryWidth / 2 - gothic.SideChapelDepth / 2,
                firstChapelCentreZ);

            StructureFootprintRect worldFootprint = StructureCardinalTransform.Rect(
                in gothic.Footprint.Primary, facing);
            int2 min = new int2(origin.x + worldFootprint.Min.x, origin.z + worldFootprint.Min.y);
        ...
        public void GothicAuthoringIsDeterministicAndBoundedInOperationCount()
        {
            ...
            CathedralAuthoring.Author(components, a, origin, in gothic);
            CathedralAuthoring.Author(components, b, origin, in gothic);
        ...
        private static bool IsReachable(HashSet<int2> solid, int2 start, int2 target, int2 min, int2 size)
        {
            int2 max = min + size;
        ...
        private readonly struct BoxWrite
        {
            ...
            public BoxWrite(int3 min, int3 size, byte material) { Min = min; Size = size; Material = material; }
        }
        ...
        private sealed class RecordingSession : IStructureAuthoringSession
        {
            ...
            public bool ContainsEmptyBox(int3 min, int3 size)
            {
                foreach (BoxWrite box in Boxes)
                    if (box.Material == GameMaterialIds.Empty && box.Min.Equals(min) && box.Size.Equals(size)) return true;
                return false;
            }
            ...
            public void Cylinder(int cx, int baseY, int cz, int radius, int height, byte material,
                int innerRadius = 0) => Operations.Add($"cyl:{cx}:{baseY}:{cz}:{radius}:{height}:{innerRadius}:{material}");
            ...
            public void Crenellate(int3 start, int3 step, int count, int width, int height, int merlon, int gap,
                byte material) { }
        ...
        private sealed class SliceSession : IStructureAuthoringSession
        {
            ...
            public void Set(int x, int y, int z, byte material) { if (y == _sliceY) Apply(new int2(x, z), material); }
            public void SetStyled(int x, int y, int z, byte material, ushort surfaceStyle,
            ...
            public void Box(int3 min, int3 size, byte material)
            {
                ...
                for (int x = min.x; x < min.x + size.x; x++) Apply(new int2(x, z), material);
            }
            public void HollowBox(int3 min, int3 size, int thickness, byte material, bool floor, bool ceiling)
            {
                if (_sliceY < min.y || _sliceY >= min.y + size.y) return;
                ...
                int innerRadius = 0)
            {
                ...
                for (int x = cx - radius; x <= cx + radius; x++)
                {
                    ...
                    if (d2 <= outer2 && (innerRadius <= 0 || d2 >= inner2)) Apply(new int2(x, z), material);
                }
            }
            public void Disc(int cx, int y, int cz, int radius, byte material) { }
            public void Cone(int cx, int baseY, int cz, int radius, int height, byte material) { }
            ...
            { if (material == GameMaterialIds.Empty) Solid.Remove(cell); else Solid.Add(cell); }
        }
    }
}
