...
namespace VoxelEngine.Showcase
{
    public sealed partial class ShowcaseWorld
    {
        ...
        private void ApplyBakedCastleSemanticRepairs()
        {
            ...
            // LoadBake captured the permanent far representation before the compatibility repair.
            // Re-scan only the already-known castle footprint so lowered-water height/material
            // metadata and its Version reflect the repaired authoritative voxels before the first
            // far mesh is published. This is the same bounded coarse pass used when the castle
            // originally finishes authoring: 256 columns per region, once at startup, never/frame.
            for (int i = 0; i < _castleRegions.Count; i++)
                CaptureFarField(_castleRegions[i]);
