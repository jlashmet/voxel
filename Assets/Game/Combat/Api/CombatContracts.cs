...
namespace Game.Combat.Api
{
    public readonly struct CombatSessionId : IEquatable<CombatSessionId>
    {
        ...
        public CombatSessionId(int value) { if (value < 0) throw new ArgumentOutOfRangeException(nameof(value)); Value = value; }
    ...
    public readonly struct CombatParticipantId : IEquatable<CombatParticipantId>
    {
        ...
        public CombatParticipantId(string value) { if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException("Participant id is required.", nameof(value)); Value = value; }
    ...
    public enum CombatTeam { Player = 0, Enemy = 1 }
    ...
    public sealed class CombatParticipant
    {
        ...
        { if (!id.IsValid) throw new ArgumentException("Participant id is required.", nameof(id)); Id = id; CharacterId = default; Team = team; }
        public CombatParticipant(CombatParticipantId id, CharacterId characterId, CombatTeam team)
        { if (!id.IsValid) throw new ArgumentException("Participant id is required.", nameof(id)); if (!characterId.IsValid) throw new ArgumentException("Character id is required.", nameof(characterId)); Id = id; CharacterId = characterId; Team = team; }
        public static CombatParticipant FromCharacter(CharacterId characterId, CombatTeam team)
        { if (!characterId.IsValid) throw new ArgumentException("Character id is required.", nameof(characterId)); return new CombatParticipant(new CombatParticipantId(characterId.Value), characterId, team); }
    }
    ...
    public sealed class CombatEncounterRequest
    {
        ...
        public CombatEncounterRequest(string encounterKey, IReadOnlyList<CombatParticipant> participants)
        {
            ...
            for (int i = 0; i < participants.Count; i++) copy[i] = participants[i] ?? throw new ArgumentException("Combat participant cannot be null.", nameof(participants));
            EncounterKey = encounterKey; Participants = Array.AsReadOnly(copy);
        }
    }

    public enum CombatLifecycleState { Idle = 0, Active = 1, Completed = 2 }
    ...
    public interface ICombatTacticalDriver { bool Step(); }
}
