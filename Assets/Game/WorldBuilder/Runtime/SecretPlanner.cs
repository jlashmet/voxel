...
{
    /// <summary>
    /// Selects generated hidden-space opportunities for a secret policy. A destroyable false wall
    /// is valid only when it actually seals normal traversal before opening, becomes traversable
    /// after opening, is destructible, non-critical, and can visually match its host surface.
    /// </summary>
    public static class SecretPlanner
    {
            ...
            uint worldSeed)
        {
            ...
            IReadOnlyList<SecretCandidate> supplied = provider.GetCandidates(site)
            ...
            for (var i = 0; i < supplied.Count; i++)
            {
                ...
                if (!candidate.Site.Equals(site))
                    continue;
                if (policy.RequiresHiddenSpace && !candidate.HiddenFromNormalTraversal)
                    continue;

                if (!TrySelectEntrance(policy, candidate, out SecretEntranceCandidate entrance))
                    continue;
            ...
            out SecretEntranceCandidate selected)
        {
            ...
            for (var i = 0; i < candidate.Entrances.Count; i++)
            {
                ...
                if (!AllowsEntrance(policy, entrance.Type))
                    continue;
                if (!EntranceSatisfiesTopology(entrance))
                    continue;
        ...
        {
            if (!entrance.SeparatesHiddenSpaceBeforeOpen)
                return false;
            if (!entrance.GrantsNormalTraversalAfterOpen)
                return false;
            if (entrance.IsStructurallyCritical)
                return false;

            if (entrance.Type == SecretEntranceType.DestroyableFalseWall)
            {
                if (!entrance.SupportsDestruction)
                    return false;
                if (!entrance.CanMatchHostSurface)
                    return false;
            }
        ...
        private static bool AllowsEntrance(SecretPolicySpec policy, SecretEntranceType type)
        {
            for (var i = 0; i < policy.EntranceTypes.Count; i++)
                if (policy.EntranceTypes[i] == type)
                    return true;
            return false;
        }
