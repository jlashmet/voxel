...
namespace Game.Vitality.Runtime
{
    public sealed class VitalityRegistry : IVitalityService
    {
        ...
        public IReadOnlyList<VitalitySnapshot> GetAll() => Capture();
        ...
        public DamageResult ApplyDamage(DamageRequest request)
        {
            ...
            if (request.Amount <= 0)
                return new DamageResult(false, DamageRejectionReason.InvalidAmount, 0, current, false);
            if (current.IsDefeated)
                return new DamageResult(false, DamageRejectionReason.AlreadyDefeated, 0, current, false);

            int applied = Math.Min(request.Amount, current.Current);
            int remaining = current.Current - applied;
            var next = new VitalitySnapshot(current.CharacterId, remaining, current.Maximum, remaining == 0, current.Revision + 1UL);
            bool defeatOccurred = !current.IsDefeated && next.IsDefeated;
            _states[request.Target] = next;
            var result = new DamageResult(true, DamageRejectionReason.None, applied, next, defeatOccurred);
            if (defeatOccurred) Defeated?.Invoke(new DefeatEvent(request.Target, next));
            return result;
        }
        ...
        public VitalitySnapshot[] Capture()
        {
            ...
            int index = 0;
            foreach (VitalitySnapshot snapshot in _states.Values) snapshots[index++] = snapshot;
        ...
        public VitalityRestoreResult Restore(VitalitySnapshot[] snapshots)
        {
            ...
            {
                VitalitySnapshot snapshot = snapshots[i];
            ...
            _states.Clear();
            foreach (KeyValuePair<CharacterId, VitalitySnapshot> pair in replacement) _states.Add(pair.Key, pair.Value);
            return new VitalityRestoreResult(true, VitalityRestoreRejectionReason.None);
        }

        private static int CompareSnapshots(VitalitySnapshot left, VitalitySnapshot right) => left.CharacterId.CompareTo(right.CharacterId);
    }
}
