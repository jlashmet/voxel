...
namespace Game.Story.Runtime
{
    ...
    public static class StoryRuleEngine
    {
        ...
        private static bool TriggerMatches(IStoryTriggerSpec trigger, StoryEvent storyEvent)
        {
            ...
            if (trigger is InteractWithNpcTriggerSpec interact) return storyEvent.Kind == StoryEventKind.NpcInteracted && interact.Npc.Equals(storyEvent.Npc);
            if (trigger is EnterSiteTriggerSpec entered) return storyEvent.Kind == StoryEventKind.SiteEntered && entered.Site.Equals(storyEvent.Site);
            if (trigger is CutsceneCompletedTriggerSpec completed) return storyEvent.Kind == StoryEventKind.CutsceneCompleted && completed.Cutscene.Equals(storyEvent.Cutscene);
            if (trigger is QuestCompletedTriggerSpec questCompleted) return storyEvent.Kind == StoryEventKind.QuestCompleted && questCompleted.Quest.Equals(storyEvent.Quest);
