...
namespace Game.Story.Runtime
{
    ...
    public static class StoryRuleEngine
    {
        ...
        private static bool TriggerMatches(IStoryTriggerSpec trigger, StoryEvent storyEvent)
        {
            ...
            if (trigger is EnterSiteTriggerSpec entered)
                return storyEvent.Kind == StoryEventKind.SiteEntered
                    && entered.Site.Equals(storyEvent.Site);
            ...
            IStoryStateView state)
        {
            for (var i = 0; i < conditions.Count; i++)
            {
                ...
                {
                    if (!state.IsCutsceneCompleted(completed.Cutscene))
                        return false;
                    continue;
                }
