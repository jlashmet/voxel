...
namespace VoxelEngine.Tests.PlayMode
{
    public sealed class MountainDragonProductionAcceptanceTests
    {
        ...
        [TestCase(false)]
        [TestCase(true)]
        public void StartupBakeContainsCurrentLandformSamples(bool dragonSample)
        {
            ...
            MountainLandformSurface surface = ShowcaseMountainDragonLayout.CreateSurface(bake.Seed);
            MountainLandformSpec spec = surface.Spec;
            MountainLandformMass summit = surface.GetMass(0);

            int sampleY = dragonSample
                ? summit.TopYdm + 1 + ShowcaseMountainDragonLayout.PlaceholderSize / 2
                : spec.OriginYdm + Math.Max(1, (summit.TopYdm - spec.OriginYdm) / 2);
            int3 sample = new int3(summit.CentreXdm, sampleY, summit.CentreZdm);
            ...
            Assert.IsTrue(record.HasValue, $"Landform sample {sample} must be inside the baked startup region set.");
            ...
            using var storage = VoxelEngineBootstrap.CreateStorage(1, Math.Max(1, mixed));
            ...
            Assert.IsTrue(cell.IsSolid,
                $"Current Mountain Dragon sample {sample} is air in the startup bake. Authored catalogue and detailed world disagree.");
        }
