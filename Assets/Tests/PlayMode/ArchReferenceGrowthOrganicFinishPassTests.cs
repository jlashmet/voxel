...
{
    public sealed class ArchReferenceGrowthOrganicFinishPassTests
    {
        ...
        public IEnumerator OrganicFinishBuildsOverlappingIvyAndDelicateBouquetsAcrossRebuild()
        {
            var host = new GameObject("Arch organic finish regression");
            try
            {
                ...
                for (int frame = 0; frame < 56 && !topology.TopologyCleanupApplied; frame++) yield return null;
                Assert.That(topology.TopologyCleanupApplied, Is.True);
                ArchReferenceGrowthOrganicFinishPass organic = host.AddComponent<ArchReferenceGrowthOrganicFinishPass>();
                for (int frame = 0; frame < 72 && !organic.OrganicFinishApplied; frame++) yield return null;
                Assert.That(organic.OrganicFinishApplied, Is.True);

                AssertOrganicPresentation(growth);
                ...
                for (int frame = 0; frame < 92; frame++)
                {
                    if (growth.HeroIvyMesh != null && growth.HeroIvyMesh != firstIvy &&
                        ...
                        topology.TopologyCleanupApplied && organic.OrganicFinishApplied)
                        break;
                    yield return null;
                }
        ...
        private static void AssertOrganicPresentation(ArchReferenceGrowth growth)
        {
            ...
            for (int cluster = 0; cluster < IvyClusterCount; cluster++)
                Assert.That(Vector2.Distance(clusterCentres[cluster], ArchReferenceGrowthAaaPass.Support(cluster)), Is.LessThan(0.10f));

            float leafRadius = AverageLeafRadius(ivy, leafStarts);
            float leafSpread = AverageLeafCentreSpread(ivy, leafStarts);
            Assert.That(leafRadius, Is.InRange(0.16f, 0.23f),
                "Leaves must stay readable without returning to oversized shelf/blob silhouettes.");
            Assert.That(leafSpread, Is.InRange(0.14f, 0.24f),
                "Each support should be a compact layered mass rather than a sparse chain.");
            Assert.That(leafRadius / leafSpread, Is.GreaterThan(0.80f),
                "Leaf size must materially overlap the support footprint; low ratios reproduce the sparse stamp failure.");

            float flowerRadius = AverageFlowerHeadRadius(petals);
            Assert.That(flowerRadius, Is.InRange(0.105f, 0.155f),
                "Heads should read as delicate blossoms, not the oversized radial rosettes from the rejected frame.");
            Assert.That(MaximumFlowerHeadAnchorOffset(petals), Is.LessThan(0.18f));
            Color averagePetal = AverageColor(petals.colors);
            Assert.That(averagePetal.r - averagePetal.b, Is.GreaterThan(0.025f),
                "Reference bouquets are predominantly warm-white/blush rather than lavender-dominant stamps.");
        ...
        {
            Vector3[] vertices = mesh.vertices;
            Vector3[] centres = MeasureClusterCentres(mesh, starts);
            float sum = 0f;
            int count = 0;
            for (int cluster = 0; cluster < IvyClusterCount; cluster++)
            for (int leaf = 0; leaf < IvyLeavesPerCluster; leaf++)
            {
                sum += Vector2.Distance(centres[cluster], vertices[starts[cluster, leaf]]);
                count++;
            }
            return sum / count;
        }
        ...
        private static float MaximumFlowerHeadAnchorOffset(Mesh petals)
        {
            ...
            {
                int bouquet = head / 5;
                max = Mathf.Max(max, Vector2.Distance(
                    MeasureFlowerHeadCentre(vertices, head),
                    ArchReferenceGrowthAaaPass.BouquetAnchor(bouquet)));
            }
            return max;
        }
