...
namespace VoxelEngine.Tests.EditMode
{
    public sealed class KentridgeMarketPiazzaTests
    {
        ...
        [Test]
        public void PiazzaInterfacesPaintBordersAndOverlapMarketStallSupports()
        {
            VoxelWorldGenSettings settings = BuildSettings();
            FeatureCatalogue piazza = KentridgeMarketPiazzaCatalogue.Build(
                Seed, settings, Allocator.Temp);
            FeatureCatalogue dressing = KentridgeVerticalPlacementAdapter.BuildTownDressing(
                Seed, settings, Allocator.Temp);
            try
            {
                FeatureDefinition definition = piazza.Definitions[0];
                int o = definition.ProgramOffset;
                const int emitBoxInstructionLength = 12;

                Assert.AreEqual((int)PrimitiveMode.Fill, piazza.Program[o + 11],
                    "The full backing slab must remain the piazza's sole geometric floor owner.");
                for (int primitive = 1; primitive < 5; primitive++)
                {
                    int p = o + primitive * emitBoxInstructionLength;
                    Assert.AreEqual((int)ShapeOp.EmitBox, piazza.Program[p]);
                    Assert.AreEqual((int)PrimitiveMode.PaintSolid, piazza.Program[p + 11],
                        "Decorative border bands must repaint the backing slab rather than contribute a coplanar geometric boundary.");
                }
                ...
                PlacementRule marketStalls = dressing.Rules[0];
                Assert.AreEqual(4, marketStalls.ExplicitCount,
                    "The first town-dressing rule should remain the four market stalls exercised by the capture.");
                for (int i = 0; i < marketStalls.ExplicitCount; i++)
                {
                    ExplicitPlacement placement = dressing.ExplicitPlacements[
                        marketStalls.ExplicitOffset + i];
                    Assert.AreEqual(surfaceY - settings.VoxelsPerDecimetre, placement.Position.y,
                        "Market-stall shoes must overlap the piazza by one authored decimetre instead of merely touching its top plane.");
                }
            }
            finally
            {
