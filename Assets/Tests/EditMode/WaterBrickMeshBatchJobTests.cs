...
namespace VoxelEngine.Tests.EditMode
{
    public sealed class WaterBrickMeshBatchJobTests
    {
        ...
        {
            using var brickBases = new NativeArray<int3>(1, Allocator.TempJob);
            using var materials = new NativeArray<byte>(
                WaterBrickMeshBatchJob.SnapshotStride, Allocator.TempJob,
                NativeArrayOptions.ClearMemory);
            using var mask = new NativeArray<byte>(
                WaterBrickMeshBatchJob.FaceArea, Allocator.TempJob,
                NativeArrayOptions.ClearMemory);
            using var vertices = new NativeList<SmoothSurfaceVertex>(4096, Allocator.TempJob);
            using var indices = new NativeList<uint>(8192, Allocator.TempJob);
