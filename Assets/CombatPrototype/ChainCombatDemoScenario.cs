...
namespace MountingForce.CombatPrototype
{
    ...
    ///
    /// The scenario deliberately exercises the combat identity in four handoffs:
    /// P1 Stephen launches the Ogre, P2 Weldon redirects the airborne body, P3 Madeline
    /// repulses the collision victim into a tree, and P4 Grom turns that tree impact into
    /// an environmental finisher.
    /// </summary>
    public sealed class ChainCombatDemoScenario
    {
        ...
        public string CurrentStepLabel
        {
            get
            {
                ...
                {
                    case 0:
                        return "P1 Stephen: Uppercut the Ogre to create an airborne event.";
                    case 1:
                        return "P2 Weldon: claim the airborne event and redirect it east.";
                    case 2:
                        return "P3 Madeline: claim the collision and repulse the Goblin into the tree.";
                    case 3:
                        return "P4 Grom: claim the tree impact and drop the tree back through the enemies.";
                    default:
                        return "Showcase complete: four players, one causal chain.";
                }
            }
        }
        ...
        private bool TryLaunch()
        {
            ...
            if (stephen == null || ogre == null)
            {
                return Fail("The showcase requires living Stephen and Ogre units.");
            }

            if (!_board.TryUppercut(stephen.Id, ogre.Id))
            {
                return Fail(_board.LastMessage);
            }
        ...
        private bool TryRedirect()
        {
            ...
            if (target == null)
            {
                return Fail("The airborne showcase target no longer exists.");
            }
        ...
        private bool TryRepulseIntoTree()
        {
            ...
            if (target == null)
            {
                return Fail("The showcase collision no longer contains a living Goblin target.");
            }
        ...
        private bool TryDropTree()
        {
            ...
            {
                return Fail("The impacted showcase tree is no longer standing.");
            }
        ...
        private ChainUnitState ChooseGoblinParticipant(ChainReactionOpportunity reaction)
        {
            ...
            if (primary != null && primary.IsAlive && primary.Kind == ChainRecruitKind.Goblin)
            {
                return primary;
            }

            if (secondary != null && secondary.IsAlive && secondary.Kind == ChainRecruitKind.Goblin)
            {
                return secondary;
            }
        ...
        private ChainUnitState FindUnit(ChainRecruitKind kind, CombatTeam team)
        {
            for (int i = 0; i < _board.Units.Count; i++)
            {
                ...
                if (unit.Kind == kind && unit.Team == team && unit.IsAlive)
                {
                    return unit;
                }
            }
