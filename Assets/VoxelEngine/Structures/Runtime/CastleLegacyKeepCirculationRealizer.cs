...
{
    /// <summary>
    /// Compatibility-only realization of the historical south-facing keep circulation recipe.
    /// Spatial castles consume CastleKeepCirculationPlan through CastleKeepCirculationRealizer;
    /// this component exists only so the legacy CastlePlan build path does not keep geometry in
    /// the stage coordinator while old callers are still supported.
    /// </summary>
