...
namespace VoxelEngine.Net.Server
{
    ...
    {
        /// <summary>Bricks per region along one axis.</summary>
        private const int k_BricksPerAxis = VoxelDimensions.RegionEdge;

        /// <summary>64³ bricks / 64 bits per word = 4096 words per region mask.</summary>
        public const int WordsPerRegionMask = VoxelDimensions.BricksPerRegion / 64;
        ...
        public void ProtectBrick(int3 regionCoord, int brickX, int brickY, int brickZ)
        {
            ...
            int bit = Region.BrickIndex(brickX, brickY, brickZ);
        ...
        public bool IsProtected(int3 worldVoxel)
        {
            ...
            VoxelAccess.Decompose(worldVoxel, out int3 regionCoord, out int3 brickCoord, out _);

            if (!_regionToMaskOffset.TryGetValue(regionCoord, out int offset))
                return false;

            int bit = Region.BrickIndex(brickCoord.x, brickCoord.y, brickCoord.z);
