...
namespace VoxelEngine.Net.Server
{
    ...
    public static class LateJoin
    {
        ...
        private const byte k_TopLevelMipIndex = 6; // 2^6 = 64 brick edge → single-cell per region at this level
        ...
        public static void HandleNewPlayer(
            ...
            ref Server server)
        {
            // Determine the player's starting region from spawn position.
            var spawnVoxel = new int3(
                (int)math.floor(spawnPosition.x),
                (int)math.floor(spawnPosition.y),
                (int)math.floor(spawnPosition.z));
            VoxelAccess.Decompose(spawnVoxel, out int3 playerRegion, out _, out _);
        ...
        public static byte[] ShipTopLevelMips(int3 playerRegionCoord)
        {
            ...
            int totalCells = k_TopLevelMipIndex >= VoxelDimensions.RegionEdgeLog2 ? 1 : 0;
            ...
            for (int dx = -k_NeighborRadius; dx <= k_NeighborRadius; dx++)
            {
                for (int dz = -k_NeighborRadius; dz <= k_NeighborRadius; dz++)
                {
                    ...
                    if (totalCells > 0)
                    {
                        ...
                        // is entirely empty/sky (0). In production this comes from the Region's
                        // occupancyMips[topLevel] value which is always resident.
