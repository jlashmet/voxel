...
namespace VoxelEngine.Vegetation.Runtime
{
    ...
    public static class ProceduralTreeSkeletonBuilder
    {
        ...
        public static TreeSkeletonSnapshot Generate(in TreeInstance instance)
        {
            ...
            for (int i = 0; i < trunkSections; i++)
            {
                ...
                float bend = profile.Gnarliness *
                             (instance.Species == TreeSpecies.Sakura ? 0.13f : 0.07f);
                ...
                skeleton.Branches.Add(new TreeBranchSegment
                {
                    Start = trunkNodes[i],
                    End = next,
                    RadiusStart = r0,
                                       ...
                                       float radius, int level, float scale)
        {
            ...
            for (int section = 0;
                 section < sections && skeleton.Branches.Count < MaxBranchSegments;
                 section++)
            {
            ...
            for (int child = 0;
                 child < childCount && skeleton.Branches.Count < MaxBranchSegments;
                 child++)
            {
                ...
                float childLength = length * profile.BranchLengthDecay *
                                    rng.NextFloat(0.82f, 1.14f);
                float childRadius = radius * profile.BranchRadiusDecay;
                GrowBranch(skeleton, ref rng, in profile, species, childStart,
                           childDirection, childLength, childRadius, level + 1, scale);
            }
                                           ...
                                           float scale, int count)
        {
            ...
            {
                float3 random = new(rng.NextFloat(-1f, 1f),
                ...
                float4 colour = math.lerp(profile.LeafColourA, profile.LeafColourB,
                                          rng.NextFloat());
                ...
                skeleton.Leaves.Add(new TreeLeafAnchor
                {
        ...
        private static void ResolveTopology(TreeSkeletonBuildState skeleton)
        {
            ...
            for (int i = 0; i < branchCount; i++)
            {
                ...
                for (int j = i - 1; j >= 0; j--)
                {
                    TreeBranchSegment candidate = skeleton.Branches[j];
            ...
            for (int i = 0; i < skeleton.Leaves.Count; i++)
            {
                ...
                {
                    TreeBranchSegment branch = skeleton.Branches[j];
        ...
        private static float3 RandomPerpendicular(ref Random rng, float3 direction)
        {
            ...
            float3 reference = math.abs(direction.y) < 0.92f
                ? new float3(0f, 1f, 0f)
                : new float3(1f, 0f, 0f);
            float3 u = math.normalizesafe(math.cross(direction, reference),
                                         new float3(1f, 0f, 0f));
            float3 v = math.normalizesafe(math.cross(direction, u),
                                         new float3(0f, 0f, 1f));
