...
{
    /// <summary>
    /// Tests whether a thin SampleField wrapper that materializes the density return value protects
    /// the existing direct UAV assignment call shape from the Metal compiler failure.
    /// </summary>
    public sealed class GpuWrappedSampleAssignmentProbeTests
    {
        ...
        [Test]
        public void MaterializedWrapperPreservesInlineUavAssignment()
        {
            if (!SystemInfo.supportsComputeShaders)
                Assert.Ignore("No compute support on this device; the wrapper probe cannot run.");
            ...
            int kernel = shader.FindKernel("CSProbeWrappedInlineSampleAssignment");
            ...
            Assert.AreEqual(0.5f, densities[0], 1e-4f,
                "Materialized SampleField wrapper did not protect direct UAV assignment: " + result);
