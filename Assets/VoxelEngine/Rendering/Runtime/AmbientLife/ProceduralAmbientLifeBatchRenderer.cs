...
namespace VoxelEngine.Rendering.Runtime.AmbientLife
{
    ...
    /// them with one instanced billboard shader. Clusters remain authoritative/network-friendly;
    /// individual insects, birds, frogs and wisps are local presentation only.
    /// </summary>
        ...
        public void SetClusters(IReadOnlyList<AmbientLifeCluster> clusters)
        {
            ...
            for (int clusterIndex = 0; clusterIndex < clusters.Count; clusterIndex++)
            {
                ...
                for (int i = 0; i < cluster.Count; i++)
                {
                    ...
                    // Sunflower placement gives organic distribution without random pile-ups. Keep
                    // every agent away from the exact centre so large winged forms cannot merge into
                    // one dominant blob even in small populations.
        ...
        private static void GetShapeScale(AmbientVisualShape shape, out float width, out float height)
        {
            switch (shape)
            {
                case AmbientVisualShape.Butterfly:
                    width = 1.42f; height = 0.90f; return;
                case AmbientVisualShape.CompactInsect:
                    width = 1.28f; height = 0.76f; return;
                case AmbientVisualShape.Dragonfly:
                    width = 1.55f; height = 1.02f; return;
                case AmbientVisualShape.GroundInsect:
                    width = 0.82f; height = 1.08f; return;
                case AmbientVisualShape.Frog:
                    width = 1.30f; height = 0.82f; return;
                case AmbientVisualShape.BirdOrBat:
                    width = 1.55f; height = 0.78f; return;
                case AmbientVisualShape.Spore:
                    width = 0.72f; height = 0.72f; return;
                case AmbientVisualShape.Wisp:
                    width = 0.72f; height = 1.30f; return;
                case AmbientVisualShape.Emberfly:
                    width = 1.18f; height = 0.88f; return;
