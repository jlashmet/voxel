    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings i):SV_Target
            {
                ...
                float ribbonA=smoothstep(0.66,0.83,ribbonNoise+0.19*sin(poolUvA.y*106.0+poolUvA.x*6.0));
                float ribbonB=smoothstep(0.70,0.87,ribbonNoise2+0.16*sin(poolUvB.y*151.0-poolUvB.x*9.0));
                float organicFoam=saturate((ribbonA*0.82+ribbonB*0.52)*pool);
                color=lerp(color,_FoamColor.rgb,organicFoam*0.58);
                float marksA=brushMark(poolUvA,15.0,42.0,1.4,0.84);
                float marksB=brushMark(poolUvB+float2(0.04,0.02),24.0,67.0,7.8,0.89);
                float marksC=brushMark(poolUvA+float2(-0.06,0.03),10.0,31.0,16.2,0.87);
                float poolWhite=saturate((marksA*0.82+marksB*0.62+marksC*0.50)*pool);
                ...
                color=lerp(color,_FoamColor.rgb,lip*lipSegments*0.58);
