...
namespace VoxelEngine.CI
{
    ...
    /// the production hard-surface extractor supplies the geometry rendered into the PNG.
    /// </summary>
    public static class KentridgeCapture
    {
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                camera.backgroundColor = new Color(0.60f, 0.70f, 0.82f, 1f);
                ...
                float centreY = TerrainSampler.HeightAt((minX + maxX) / 2,
                                                         (minZ + maxZ) / 2, Seed) * VoxelSize + 11f;
                Vector3 focus = new Vector3(centreX, centreY, centreZ);
                float spanMetres = Mathf.Max(maxX - minX, maxZ - minZ) * VoxelSize;
                float distance = Mathf.Max(120f, spanMetres * 1.48f);
                Vector3 viewDirection = new Vector3(0.92f, 0.78f, -1.05f).normalized;
                cameraObject.transform.position = focus + viewDirection * distance;
                ...
                terrainMaterial = NewMaterial(lit, "CI Kentridge Terrain", new Color(0.25f, 0.43f, 0.20f, 1f));
                terrainRenderer.sharedMaterial = terrainMaterial;

                palette = BuildPalette(lit);
                ...
                lightObject = new GameObject("CI Kentridge Sun");
                Light sun = lightObject.AddComponent<Light>();
                sun.type = LightType.Directional;
                sun.intensity = 1.25f;
                sun.color = new Color(1.0f, 0.94f, 0.84f, 1f);
                lightObject.transform.rotation = Quaternion.Euler(48f, -32f, 0f);

                target = new RenderTexture(Width, Height, 24, RenderTextureFormat.ARGB32)
                {
                    name = "CI Kentridge Overview",
                    antiAliasing = 4,
                };
                target.Create();
                ...
                try
                {
                    ...
                    File.WriteAllBytes(Path.Combine(outputDirectory, "kentridge-overview.png"),
                                       capture.EncodeToPNG());
                }
                finally
                {
                ...
                Debug.Log($"CI Kentridge capture written to {outputDirectory}\n{metadata}");
            }
        ...
        private static Mesh MeshEntry(CpuHardSurfaceChunkCache.Entry entry, out int triangleCount)
        {
            ...
            var mesh = new Mesh { name = $"CI Kentridge {entry.Coordinate}", indexFormat = IndexFormat.UInt32 };
            mesh.vertices = vertices;
        ...
        private static Material[] BuildPalette(Shader shader)
        {
            ...
            colours[1] = new Color(0.60f, 0.57f, 0.50f, 1f); // stone
            colours[2] = new Color(0.34f, 0.20f, 0.10f, 1f); // timber
            colours[3] = new Color(0.70f, 0.62f, 0.44f, 1f);
            colours[4] = new Color(0.45f, 0.70f, 0.82f, 0.85f); // glass
            colours[6] = new Color(0.30f, 0.31f, 0.33f, 1f); // dark masonry
            colours[7] = new Color(0.25f, 0.29f, 0.36f, 1f); // slate
            colours[8] = new Color(0.55f, 0.22f, 0.13f, 1f); // roof tile
            colours[9] = new Color(0.45f, 0.22f, 0.52f, 1f); // cloth
            colours[10] = new Color(0.28f, 0.52f, 0.22f, 1f);
            colours[13] = new Color(0.40f, 0.30f, 0.20f, 1f); // road
            colours[14] = new Color(0.20f, 0.40f, 0.18f, 1f); // moss
            colours[15] = new Color(0.92f, 0.70f, 0.30f, 1f); // warm window
