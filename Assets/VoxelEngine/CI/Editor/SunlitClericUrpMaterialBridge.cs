...
namespace VoxelEngine.CI
{
    ...
    internal static class SunlitClericUrpMaterialBridge
    {
        ...
        private static void HideShowcaseScenery()
        {
            ...
            foreach (GameObject go in Resources.FindObjectsOfTypeAll<GameObject>())
            {
                ...
                if (go.name == "Waterfall Body" || go.name == "Waterfall Sun Streak" ||
                    ...
                    go.name == "Foreground Stream" || go.name == "Sunlit Oak" || go.name == "Far Garden Tree")
                    go.SetActive(false);
            }
        }
        ...
        {
            // Narrow the mannequin proportions without changing the vertical frame coverage.
            ...
            if (leftEye != null) leftEye.localScale = new Vector3(0.055f, 0.044f, 0.024f);
            if (rightEye != null) rightEye.localScale = new Vector3(0.055f, 0.044f, 0.024f);
            ...
            // Layered rear hair + two front swoops frame the face without pigtails.
            Vector3[] hairTop =
            {
            ...
            for (int i = 0; i < hairTop.Length; i++) CapsuleLocal($"Long Hair {i}", hairTop[i], hairBottom[i], 0.060f, hair, hero);
            CapsuleLocal("Front Hair Left", new Vector3(-0.04f, 1.78f, -0.13f), new Vector3(-0.19f, 1.61f, -0.245f), 0.040f, hair, hero);
            CapsuleLocal("Front Hair Right", new Vector3(0.04f, 1.78f, -0.13f), new Vector3(0.19f, 1.61f, -0.245f), 0.040f, hair, hero);

            Mesh cape = new Mesh { name = "Sunlit Cleric Cape" };
            cape.vertices = new[]
            {
                new Vector3(-0.20f, 1.20f, 0.10f), new Vector3(0.20f, 1.20f, 0.10f),
                new Vector3(0.46f, 0.30f, 0.16f), new Vector3(-0.46f, 0.30f, 0.16f),
            };
            ...
            // Mirror the staff to viewer-left, where it sits in the reference, and shrink the
            // oversized symbol to roughly the width of Madeline's head.
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                CapsuleLocal($"Staff Ring {i}", p0, p1, 0.012f, gold, hero);
            }
        }
        ...
        private static void BuildReferenceDiorama(Vector3 hero)
        {
            ...
            Material stone = Lookdev("Reference Warm Stone", "stone", new Color(0.72f, 0.65f, 0.52f), 0.12f);
            Material darkStone = Lookdev("Reference Dark Stone", "rock", new Color(0.43f, 0.42f, 0.36f), 0.08f);
            Material moss = Smooth("Reference Moss", new Color(0.38f, 0.59f, 0.18f));
            Material mossDark = Smooth("Reference Moss Dark", new Color(0.24f, 0.44f, 0.12f));
            ...
            Material water = Transparent("Reference Water", new Color(0.15f, 0.70f, 0.86f, 0.60f), new Color(0.025f, 0.11f, 0.16f));
            Material waterWhite = Transparent("Reference Water Highlight", new Color(0.92f, 0.99f, 1f, 0.68f), new Color(0.06f, 0.08f, 0.10f));
            Material castleStone = Smooth("Reference Castle", new Color(0.68f, 0.68f, 0.62f));
            ...
            CreateCloudCluster(hero + new Vector3(-1.15f, 1.85f, 14.5f), 0.82f, cloud);
            CreateCloudCluster(hero + new Vector3(0.45f, 2.10f, 15.5f), 0.95f, cloud);
            CreateCloudCluster(hero + new Vector3(1.90f, 1.75f, 14.0f), 0.78f, cloud);

            // Smaller, thinner hero island. Multiple moss caps below break the perfect oval.
            ...
            CreateIsland(hero + new Vector3(0.90f, 0.00f, 4.20f), new Vector3(2.10f, 0.72f, 1.95f), darkStone, moss, 7);
            ...
            // Meandering right-side stream, visible around the hero island instead of hidden under it.
            ...
            // Three narrow tiered waterfalls like the reference instead of one cyan wall.
        ...
        private static void CreateIsland(Vector3 centre, Vector3 size, Material stone, Material moss, int rimBlocks)
        {
            ...
            // Three overlapping caps create the lumpy storybook silhouette from the reference.
            ...
            for (int i = 0; i < rimBlocks; i++)
            {
                ...
                CreateBlock(centre + new Vector3(x, size.y * 0.43f, z),
                            new Vector3(size.x / rimBlocks * 1.05f, 0.30f, 0.35f),
        ...
        private static void CreateArch(Vector3 basePos, float radius, float height, Material stone, Material moss)
        {
            ...
            for (int i = 0; i < archBlocks; i++)
            {
                ...
                Vector3 p = basePos + new Vector3(Mathf.Cos(angle) * radius, springY + Mathf.Sin(angle) * radius, 0f);
