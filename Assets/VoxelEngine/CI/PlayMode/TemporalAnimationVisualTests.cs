...
namespace VoxelEngine.CI
{
    ...
    public sealed class TemporalAnimationVisualTests
    {
        ...
        private static IEnumerator CaptureAmbientSequence()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < frameCount; i++)
                {
                    ...
                    Assert.That(CountForeground(frame, camera.backgroundColor), Is.GreaterThan(300), $"Frame {i} lost most visible agents.");
                    if (i == 0) continue;
                    int count = CountChanged(frames[i - 1], frame, 0.010f);
                    changed[i - 1] = count; minChanged = Mathf.Min(minChanged, count); maxChanged = Mathf.Max(maxChanged, count); totalChanged += count;
                }

                float averageChanged = totalChanged / (float)changed.Length;
                var report = new StringBuilder();
                for (int i = 0; i < changed.Length; i++) report.AppendLine($"transition_{i:00}_{i + 1:00}_changed_pixels={changed[i]}");
                report.AppendLine($"min_changed_pixels={minChanged}"); report.AppendLine($"max_changed_pixels={maxChanged}"); report.AppendLine($"average_changed_pixels={averageChanged:0.0}");
        ...
        private static IEnumerator ValidateGrass(List<Texture2D> frames)
        {
            ...
            GameObject root = new GameObject("CI Grass Wind Validation"); Texture2D background = null;
            try
            {
                ...
                camera.Render(); background = VegetationLifeRenderingVisualTests.ReadTarget(target);
                ...
                VerticalMotion m = AnalyseVertical(first, second, background, 0.22f, 0.55f);
            ...
            finally { if (background != null) Object.DestroyImmediate(background); VegetationLifeRenderingVisualTests.ReleaseTarget(camera, target); Object.DestroyImmediate(cameraObject); Object.DestroyImmediate(root); }
        }
        ...
        private static IEnumerator ValidateVine(List<Texture2D> frames)
        {
            ...
            GameObject root = new GameObject("CI Vine Wind Validation"); Texture2D background = null;
            try
            {
                VegetationRenderingShowcase showcase = root.AddComponent<VegetationRenderingShowcase>(); yield return null;
                VegetationLifeRenderingVisualTests.RemovePresentationGeometry(root.transform); showcase.Renderer.enabled = false; showcase.Renderer.Clear();
                ...
                VerticalMotion m = AnalyseVertical(first, second, background, 0.46f, 0.78f);
        ...
        private static IEnumerator ValidateWoody(List<Texture2D> frames)
        {
            ...
            GameObject root = new GameObject("CI Woody Static Validation"); Texture2D background = null;
            try
            {
                VegetationRenderingShowcase showcase = root.AddComponent<VegetationRenderingShowcase>(); yield return null;
                VegetationLifeRenderingVisualTests.RemovePresentationGeometry(root.transform); showcase.Renderer.enabled = false; showcase.Renderer.Clear();
                ...
                VegetationLifeRenderingVisualTests.ForegroundMask(first, background, out int countA); VegetationLifeRenderingVisualTests.ForegroundMask(second, background, out int countB);
                int foreground = Mathf.Max(countA, countB); int changed = CountChanged(first, second, 0.004f); float rate = changed / (float)Mathf.Max(1, foreground);
                ...
                Assert.That(foreground, Is.GreaterThan(80)); Assert.That(rate, Is.LessThan(0.01f), "Woody debris should remain static without physics interaction.");
            }
            finally { if (background != null) Object.DestroyImmediate(background); VegetationLifeRenderingVisualTests.ReleaseTarget(camera, target); Object.DestroyImmediate(cameraObject); Object.DestroyImmediate(root); }
        }
        ...
        {
            bool[] aMask = VegetationLifeRenderingVisualTests.ForegroundMask(first, background, out _); bool[] bMask = VegetationLifeRenderingVisualTests.ForegroundMask(second, background, out _);
            Color32[] a = first.GetPixels32(); Color32[] b = second.GetPixels32(); int width = first.width, height = first.height, minY = height, maxY = -1;
            ...
            float span = Mathf.Max(1f, maxY - minY); int lowerPixels = 0, lowerChanged = 0, upperPixels = 0, upperChanged = 0;
            for (int i = 0; i < aMask.Length; i++)
            {
                if (!aMask[i] && !bMask[i]) continue; float y = (i / width - minY) / span; bool changed = PixelDistanceSquared(a[i], b[i]) > 0.0025f;
        ...
        private static int CountChanged(Texture2D first, Texture2D second, float threshold)
        {
            Color32[] a = first.GetPixels32(), b = second.GetPixels32(); int changed = 0;
            for (int i = 0; i < a.Length; i++) if (PixelDistanceSquared(a[i], b[i]) > threshold) changed++;
            return changed;
        }

        private static int CountForeground(Texture2D texture, Color background)
        {
            Color32[] pixels = texture.GetPixels32(); Color32 bg = background; int count = 0;
            for (int i = 0; i < pixels.Length; i++) if (PixelDistanceSquared(pixels[i], bg) > 0.0035f) count++;
            return count;
        }
